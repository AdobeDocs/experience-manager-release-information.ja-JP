...
* コンテンツフラグメントに不要な &lt;br> を挿入したテキストをコピーペーストします。 NPR-26660：CRTE-151 のホットフィックス
...
* （OSGI） [JEE] 「署名プロセスの検証」を使用して検証する際に、添付ファイル付きのPDFのPDFステータスが正しくない。 NPR-23269、NPR-23737
...
* プライマリで移動された構造をロールアウトすると、誤った `cq:moveTarget` が発生します。 NPR-22373：CQ-4232536 のホットフィックス
...
* org.apache.sling.engine.SlingRequestProcessor を介してリクエストを処理で `org.apache.sling.i18n.DefaultLocaleResolver` ません。 NPR-20706：CQ-94880 のホットフィックス
...
* docConvertor が「The prefix &quot;stEvt&quot; for element `stEvt:action` is not bound」というエラーでPDF/A を生成できない。 NPR-21032：CQ-4222540 のホットフィックス
...
* 別のユーザーによってロックされているバージョンのページをユーザーが公開しよう `/unpublish/create` すると、AEMに予期しない動作が表示される。 NPR-19249：CQ-4215298 および CQ-4203856 のホットフィックス
...
* クラシック UI とタッチ UI のいずれかを使用してページを移動する場合、移動ダイアログに 150 を超える参照が表示されないので、ユーザーはこれらの参照を更新してページを再公開できません。この問題は、クラシック UI のプロパティ「maxRefNo」が導入され、サイト管理ノード「`'/libs/wcm/core/content/siteadmin'`」で設定できるようになったことで修正されました。 このプロパティは、大量の移動操作が行われる前に表示される、参照の最大数（デフォルト値は 150）を指定します。ページに複数の参照がある場合、それらの参照は movePage ダイアログボックスには表示されません。この設定は dam 管理者にも機能し、その他の管理者にも機能します。それぞれ `'/libs/wcm/core/content/damadmin'` ノードと `'/libs/wcm/core/content/miscadmin'` ノードに設定を適用します。 NPR-17222：CQ-85878 のホットフィックス
...
* **Assets オムニサーチ** の列の追加またはカスタマイズの結果、`/apps` でのオーバーレイは機能しません。 NPR-16737：CQ-4206785 のホットフィックス
...
* Firefox またはGoogleのChrome ブラウザーを使用して **[!UICONTROL グローバルナビゲーション]** パネルに移動すると、異なるユーザーエクスペリエンスが提供されます。 Firefox ブラウザーに「**[!UICONTROL ツール]**」メニューが表示されます。 Google Chromeブラウザーに **[!UICONTROL ナビゲーション]** メニューが表示されます。 NPR-16770：CQ-4200456 のホットフィックス
...
* アダプティブ Formsのルールエディターで **[!UICONTROL 項目を選択]** すると、イベントが「null」と表示される。 NPR-17139
...
* `dataRef=C:/Users/` デフォルトの事前入力サービス設定 **[!UICONTROL タイプの正規表現のサポート]**。 このタイプは、アダプティブFormsのデータの事前入力用です。 NPR-16425
...
* ルールエディターの構文 **[!UICONTROL フォームを保存]** が機能しません。 NPR-16905
...
* 送信済みインスタンスの場合、ファイル添付の `Upload Comments` は UI に表示されませんが、CRX リポジトリに格納された XML には存在します。 NPR-17075
...
* Output サービスは、`date-time format` メタデータに「ミリ秒」フィールドを含めるように変更されたPDFを処理できません。 NPR-16838
...
* `/apps` 下のオーバーレイファイル（JSP またはJavaScript ファイル）と、`/libs` 下のホットフィックスに含まれるファイルとの競合を検出するための、パッケージマネージャーの「検証」オプション。 その後、影響を受けるオーバーレイのベースを変更して、`/libs` の下にあるファイルからの変更を含めることができます。 NPR-16216：CQ-81729 のホットフィックス
...
* AEM 6.0 から AEM 6.2 にアップグレードした後、多数のクエリが原因でタグを検索する際に、クラシック UI のパフォーマンスが遅くなります。この問題を解決するには、[ タグ付けコンソール（クラシック UI）のレプリケーションステータスを無効にする ](#disable-replication-status-in-tagging-console-classic-ui-npr) で説明されている手順に従うことができます。 NPR-15842：CQ-4201748 のホットフィックス。
...
* アダプティブフォームを添付ファイルと共に保存する場合、添付ファイルのフルパスが、生成された XML の `fileAttachment` タグに、添付ファイルの名前ではなく追加されます。 NPR-16529
...
* 「file://」、「http://」、「ftp://」などのプロトコルを通じた不正アクセスからフォーム事前入力サービスを保護します。 詳しくは、[Configuration Manager を使用した事前入力サービスの設定 ](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) を参照してください。 NPR-15434
...
* ページコンポーネントに移動する際、`tagBasePath` プロパティを持つタグの値が、クラシック UI ダイアログボックスで取得されない。 NPR-15543：CQ-4199950 のホットフィックス
...
* ユーザーが CRXDE 内のノード（例：`/content/dam/`）を選択します。 次に、「アクセス制御」タブをクリックし、アクセス制御リストが存在することを確認します。 これで、一部の要素をドラッグ&amp;ドロップすると、選択した要素ではなく、要素が移動します。 NPR-15696：Granite-16300 のホットフィックス
...
* タイムワープモード中に編集ページを閉じ、サイト管理から再度開こうとすると、ステータス `500` でエラーが発生します。 NPR-14965：CQ-109647 のホットフィックス：
* Digital Asset Manager (DAM) UI で、「ユーザーピッカー」の「認証可能な項目を検索」を使用すると、「メモリ不足」の例外が発生します。NPR:15307：CQ-98542 のホットフィックス

### アセット {#assets-17}

* オムニサーチでアセットを検索し、アセットを選択して **プロパティを表示** をクリックしてプロパティを編集しようとすると、「**保存**」ボタンが空白のページにリダイレクトされる。 NPR-15900：CQ-4202372 のホットフィックス
...
* **[!UICONTROL タッチ UI]** では、Internet Chrome ブラウザーバージョン 56.0.2924.87 を使用している場合、type=&#39;datetime&#39;になった日付フィールドに保存された日付が表示されます。NPR-15383:GRANITE-16481 のホットフィックス
...
* `/apps` を使用しているお客様が列データソースを追加することを、オムニサーチでは許可していません。これは、ロケーション設定が */libs/granite/omnisearch/content/metadata/* にリストされていることを前提としているからです。 NPR-13188：Granite-16479 のホットフィックス
...
* Campaign - ターゲティングと `MAC` - テストと Target の統合コンポーネントを使用する際、アクティビティの非公開によってプライマリUI のアクティビティステータスが更新されない。 NPR-15401：CQ-4199839 のホットフィックス
...
* 事前入力 XML でも、XML スキーマベースのアダプティブフォームにはラッパーがありませんが `afData/afBoundData` 送信された XML には `afData/afBoundData` ラッパーが存在します。 NPR-15541
...
* エディターからページプロパティダイアログボックスが送信されると、MSM Post プロセッサーは、`msm:writeLiveCopyConfig` パラメーターではなく、リクエストからいくつかのパラメーターを書き込みます。 NPR-14434
...
* `Jquery` の更新以降、ユーザーはアセットコレクションを選択できず、コンテンツフラグメントのコンテンツを関連付けパネルで選択を確定できません。 NPR-14847：CQ-4194209 のバックポート
...
* SAML 認証ハンドラーを使用して設定された「saml_request_path」 Cookie は、`?` 文字が追加された Cookie セットを表示します。 さらに、SAML 応答が AEM にポストバックされると、AEM「saml_request_path」クッキーは、既にエンコードされている文字のため無効な値を返します。NPR-13517：Granite-11722 と Granite-14414 のホットフィックス
...
* *FormsandDocuments* ディレクトリ内を移動する際、ユーザーがアセットをコピーしてから別のフォルダーに移動すると、「貼り付け」ボタンは表示されません。 CQ-111327
...
* *`/apps`* の *&quot;/libs/cq/tagging/widgets/source/widgets/admin/TagAdmin.js&quot;* をオーバーレイ
...
1. `jsafeFIPS.jar`、`jsafeJCEFIPS.jar` および `certjFIPS.jar` ファイルのバックアップを [AEM_Forms_Installation_directory]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/` に作成し、前述のディレクトリからファイルを削除します。
...
1. グローバル設定を書き出します。
1. ダウンロードした XML ファイルで、タグ `<client_tasksPollingInterval>10</client_tasksPollingInterval>` を追加します。 10 は秒単位の例の値です。 適宜変更できます。 
1.  ファイルを保存します。
...
* リポジトリの再起動が原因で、**コンポーネントインスタンスの作成** および **サービスファクトリが null を返した** などのエラーが発生します。
