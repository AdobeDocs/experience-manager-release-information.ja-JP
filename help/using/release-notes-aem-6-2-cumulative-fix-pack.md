...
* （クラシック UI）ダイアログボックスでオーサリングすると、タグ全体が空として表示されます。NPR-24688：CQ-4246407 のホットフィックス
...
* 基盤テーブルのセキュリティをプロアクティブに修正して、デザインの一貫性を維持します。
...
* EmbedXMP データが TIFF、Pyramid 生成プロセスで常に「アクティブ」に設定されます。NPR-22776：CQ-4234498 のホットフィックス
* 複数値フィールドに複数のデフォルト値を設定できません。NPR-22900：CQ-4239000 のホットフィックス
* （Dynamic Media）「ダイナミックレンディション」チェックボックスを選択すると、ダウンロードした zip ファイルが、0 バイトのファイルを含む元の TIFF 画像を生成します。NPR-22410：CQ-4198471 のホットフィックス
...
* ページの文字化けとともにタイムワープが「RecursionTooDeepException」エラーを返し、日付が変更された場合でも新しいバージョンが作成されません。NPR-21707：CQ-4199536 のホットフィックス
...
* オムニサーチの文字が多すぎると、AEM サーバーがクラッシュするおそれがあります。NPR-21083：CQ-4223602 のホットフィックス
...
* サービス OMPFSubmission/PDFA/PDFtoPDFA を呼び出し中に、`java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` という名前の例外が発生します。これにより、サーバーが再起動されるまで、短時間のみ有効な署名の検証プロセスが完了しなくなります。NPR-20792
...
>* 改行を含まないコンテンツが CM テキストエディターで折り返されません。NPR-19449
>
...
* ページ上の画像を削除した後で、別の画像を選択できません。NPR-17711：CQ-4201323 のホットフィックス
...
* クラシック UI とタッチ UI のいずれかを使用してページを移動する場合、移動ダイアログに 150 を超える参照が表示されないので、ユーザーはこれらの参照を更新してページを再公開できません。この問題は、クラシック UI の「maxRefNo」プロパティを導入することで修正されました。これは、siteadmin ノード（/libs/wcm/core/content/siteadmin）で設定可能）です。このプロパティは、大量の移動操作が行われる前に表示される、参照の最大数（デフォルト値は 150）を指定します。ページに複数の参照がある場合、それらの参照は movePage ダイアログボックスには表示されません。この設定は、それぞれ `'/libs/wcm/core/content/damadmin'` ノードと `'/libs/wcm/core/content/miscadmin'` ノードに設定を適用することにより、damadmin と miscadmin にも適用されます。NPR-17222：CQ-85878 のホットフィックス
...
* AJAX を介してアダプティブフォームを送信中にエラーが発生しても、「error」コールバック関数が呼び出されません。NPR-17034
...
* ターゲティングを使用している場合、キャンペーンを公開しても、ブランドやプライマリを含むツリー全体は自動的には公開されません。NPR-15855：CQ-94630 のホットフィックス
...
* 複数のファイルを添付できる添付ファイルウィジェットの場合、添付ファイルを含んだ新しいフォームインスタンスが、以前の添付ファイルを持つウィジェット上で送信されると、エラーコードが表示されます。このエラーは、実際のコンテンツではなく、追加された添付ファイルを開いたときに発生します。NPR-16258
...
* スクロール可能なコンテンツを含んだレターを Internet Explorer ブラウザーでプレビュー中に、ブラウザーのスクロールバーがレターのスクロールバーと重なります。そのため、右側のフラグメントではコンテンツ全体を表示できません。NPR-16068
* Google Chrome ブラウザーを使用してテキストドキュメントフラグメントを作成または編集中に、色選択ドロップダウンが自動的にポップアップ表示され、削除できません。フラグメントを編集するには、データエントリのタイプでリストを選択する必要があります。NPR-16067
...
* 「file://」、「http://」、「ftp://」などのプロトコルを使用して、フォームの事前入力サービスを不正アクセスから保護します。詳しくは、[Configuration Manager を使用した事前入力サービスの設定](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)を参照してください。NPR-15434
...
* エディターから PageProperties ダイアログボックスを送信する場合、ライブコピータブ内の変更されていないデータが書き換えられます。NPR-14693
...
* 「条件を指定して繰り返し」式を持つアセットが正しく表示されません。`*CDN3835RLCEN*` レターテンプレートをプレビューすると、本文のターゲット領域にあるアセットが表示されません。オプションの事前に選択されているアセットである `*VIPReassement*` の選択が解除された場合は、事前選択されている他のアセットがレターに表示されます。NPR-14844
...
* チャネルエディターでビデオのビデオプロファイルを編集しようとすると、デザインダイアログが開かず、エラーログで null ポインター例外が発生します。NPR-14144：CQ-81101 のホットフィックス
...
1. [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/ にある `jsafeFIPS.jar` ファイル、`jsafeJCEFIPS.jar` ファイル、`certjFIPS.jar` ファイルのバックアップを作成し、前述のディレクトリからファイルを削除します。
...
共有キューの数は、ユーザーがタスクを要求したときに、デフォルトでは他のユーザーに対して更新されません。このため、アドビでは新しいプロパティを導入しました。次の手順に従って、AEM インスタンスでこのプロパティを設定します。
