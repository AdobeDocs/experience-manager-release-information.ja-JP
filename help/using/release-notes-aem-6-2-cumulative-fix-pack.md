...
| **前提条件** | [AEM 6.2 サービスパック 1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) |
| **一般リリース** | 2019年6月06日（PT） |
...
CFP および他のタイプのリリースの詳細については、[メンテナンスリリースの提供](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)を参照してください。

## リリースについて {#about-the-release}

AEM 累積修正パック 6.2 SP1-CFP20 は、AEM 6.2 用の最後の累積修正パックで、[AEM 6.2 SP1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) の一般リリース以降にリリースされた主なお客様向け修正を含む重要なアップデートです。
...
>[!NOTE]
>
>* AEM 累積修正パック 6.2 SP1-CFP10 には、新しい Sling `discovery- api` バンドル Johnzon 1.0.0 が含まれています。さらに、サービスユーザーの sling-discovery には、ノードの読み取りおよび書き込み権限が追加されます */var/discovery* CRX リポジトリで上書きできます。
>
>* Apache Commons のメールバンドル **org.apache.commons/commons-email/1.5** を置き換えて追加されました **com.day.commons.osgi.wrapper/com.day.commons.osgi.wrapper.commons-email/1.2.0-0002**.
>
>* Adobeでは、AEM インスタンスにユーザーが多いお客様に、インストールフォルダーを介して CFP をデプロイすることをお勧めします。
>
...
* レターをPDFとしてプレビューする際、プライマリページに配置されたテキストフィールドは、「データ」タブまたは指定されたデータリンケージに従って入力された値を受け入れません。 NPR-29239：CQ-4266856 のホットフィックス。
...
AEM 累積修正パック 6.2 SP1-CFP19 は、[AEM 6.2 SP1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) の一般リリース以降にリリースされた主なお客様向け修正を含む重要なアップデートです。

この累積修正パックの主要点は次のとおりです。

* AEM 6.2 で MS® Translator API v3.0 のサポートを有効化
...
* この `cq:actions` は、ターゲットコンポーネントでは考慮されません。 NPR-27616：CQ-4257497 のホットフィックス
...
* を使用するリソースバンドルのディクショナリは無効になりません `sling:basename`. NPR-27624
...
AEM 累積修正パック 6.2 SP1-CFP18 は、[AEM 6.2 SP1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) の一般リリース以降にリリースされた主なお客様向け修正を含む重要なアップデートです。
...
* `Camera RAW` 大量に取り込まれる間にプロセスが停止し、最終的にすべてのワークフロー処理がブロックされる。 NPR-26990：NPR-23860 のホットフィックス
* このダウンロード機能では、匿名ユーザーがすべてのアセットをダウンロードできるように、アセットダウンロードサーブレット経由でAEM Assetsを使用します。 NPR-27054：CQ-4254732 のホットフィックス
* AEM のメールテンプレートの件名行に、特殊文字が壊れて表示されます。NPR-26470：CQ-4252368 のホットフィックス

### Sites {#sites-1}

* ConfigPostProcessor クラスの動作が正しくないため、親画像を休止すると削除される `cq:LiveRelationship` 子ページからの混合タイプ。 NPR-26745：CQ-4254163 のホットフィックス
* コアページコンポーネントにリダイレクトのサポートを追加しました。NPR-26576：CQ-110529 のホットフィックス
* Context hub のへの移行 `jQuery` 3. NPR-26956：CQ-4255472 のホットフィックス
* アンカー入力フィールドは、最大化されるまで、ダイアログボックスの「表示可能なブラウザー」セクションから表示されます。 NPR-26852：CQ-4255019 のホットフィックス
...
AEM 累積修正パック 6.2 SP1-CFP17 は、[AEM 6.2 SP1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) の一般リリース以降にリリースされた主なお客様向け修正を含む重要なアップデートです。
...
* jqueryui clientlib v1.12.1 を更新します。
...
* Context hub を jQuery 3 に移行します。 NPR-25059：Granite-19902 のホットフィックス
* ネストされた Parsys コンポーネントの場合、常にデザインを満たす最初の（ネストが最小のパス）が、使用可能な複数のコンポーネントから適用されます。 詳しくは、[デザインパスの解決](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)を参照してください。NPR-25250：CQ-4246276 のホットフィックス

### 統合 {#integration-3}

* OOTB ターゲット統合を使用して、コンポーネントをターゲティングすると、空のターゲットコンポーネントではなく、ページ全体がレンダリングされます。NPR-25273：CQ-4248003 のホットフィックス
* ターゲティングモードでは、継承を解除しても、編集モードでは継承が解除されないので、コンポーネントはターゲットとして表示されます。 NPR-25324：CQ-4248162 のホットフィックス
* ページ上でパーソナライゼーションが定義され、オーディエンスが解決されると、対応するエクスペリエンスが編集モードで表示されます。 NPR-25731：CQ-4249465 のホットフィックス
* デフォルト以外のコンテキストパスで AEM を使用した場合に、誤ったティーザー URL が表示されます。NPR-25971：CQ-4250953 のホットフィックス
* オプトアウトを使用する際の空白レンダリング。 NPR-25295：CQ-4246792 のホットフィックス
* オーサー環境から削除されたエクスペリエンスは、ページのアクティベーション時に公開済みサイトから削除されません。 NPR-24869：CQ-4247832 のホットフィックス

### DAM - DM クライアント {#dam-dm-client}

* （Chrome、Firefox） VideoPlayer は、タッチ操作対応デバイスでのマウスクリックを無視します。 CQ-4247370 のホットフィックス

### Platform {#platform-1}

* パッケージの取得／解放時に最大再試行数を設定できるようにしました。NPR-25328：Granite-22376 のホットフィックス
* レプリケーションエラーが発生した場合の誤ったログ。 NPR-25308：CQ-4249402 のホットフィックス
...
* jqueryui clientlib v1.12.1 を更新します。NPR-25090:Granite-21981、CQ-4248897 のホットフィックス
...
AEM 累積修正パック 6.2 SP1-CFP16 は、[AEM 6.2 SP1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) の一般リリース以降にリリースされた主なお客様向け修正を含む重要なアップデートです。
...
* コンポーネントがパブリッシュインスタンスをターゲットにしている場合、ターゲットのエクスペリエンスより前にデフォルトのエクスペリエンスが示されるちらつきが表示されます。 NPR-23992：CQ-4242038 のホットフィックス
* オーサー環境から削除されたエクスペリエンスは、ページのアクティベーション時に公開済みサイトから削除されません。 NPR-24869：CQ-4247832 のホットフィックス
...
* ライブコピー領域でエクスペリエンスを作成するには、まず継承を壊して設定できるようにする必要があります。 NPR-24995：CQ-4248209 のホットフィックス
* （タッチ UI）ページのロックまたはロック解除をおこなうと、上部のツールバーのいくつかのアイコンが非表示になります。NPR-23954：CQ-4243345 のホットフィックス
* Context Hub のフィールドが正しく配置されていません。 NPR-23958
...
* デフォルトのプレビューを使用してレターをプレビューすると、「プレビュー」をクリックしたときにレイアウトフラグメントのマッピングが正しく表示されませんが、マッピングは正しく表示されます。 NPR-22993：CQ-4237745 のホットフィックス
...
* シェルスクリプトファイルのウィンドウ行の終端が原因で、UNIX® では LCM が動作しません。 NPR-22958
...
AEM 累積修正パック 6.2 SP1-CFP15 は、[AEM 6.2 SP1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) の一般リリース以降にリリースされた主なお客様向け修正を含む重要なアップデートです。

この累積修正パックの主要点は次のとおりです。

* 設計の一貫性を維持するために、基盤テーブルのセキュリティ修正をプロアクティブに行います。
...
* 考慮する検証フックを調整しました `:invalid` ブースト番号入力の項目。

### アセット {#assets-4}

* EmbedXMP データは、TIFF、Pyramid 生成プロセスでは常に「アクティブ」に設定されます。 NPR-22776：CQ-4234498 のホットフィックス
* 複数値フィールドに複数のデフォルト値を設定できません。NPR-22900：CQ-4239000 のホットフィックス
* （Dynamic Media）「動的レンディション」チェックボックスをオンにした場合、ダウンロードした zip ファイルから 0 バイトのファイルを含む元のTIFFが生成されます。 NPR-22410：CQ-4198471 のホットフィックス
...
* マルチフィールドコンポーネントで「項目を追加」をクリックできない。 NPR-22552：CQ-4237404 のホットフィックス
...
* デフォルトプレビューを使用してレターをプレビューすると、「プレビュー」ボタンをクリックしたときにレイアウトフラグメントのマッピングが正しく表示されず、正しく表示されません。 NPR-23335：CQ-4237745 のホットフィックス
...
* 既に有効期限切れの状態のアセットに対して、パスワード有効期限通知ジョブを再設定しました。
* より多くのロケールを拡張するようにカスタマイズされたタッチ UI コンソール。
...
* 一重引用符文字を使用すると、一括表示でメタデータを更新できなくなり、クイックツールバーアクションを使用してアセットのプロパティを開くと UI が壊れます。 NPR-22317、NPR-22353：CQ-4236990、CQ-4236469 のホットフィックス
...
* 小さい画像用の web レンディションのピクセルサイズが不正確です。 NPR-22435：CQ-4236742 のホットフィックス

### Sites {#sites-5}

* （タッチ UI）移動されたタグが、ページプロパティの古い場所と新しい場所に表示される。 NPR-21921：CQ-4238598 のホットフィックス
...
* （Digital Publishing Suite）公開日とカバー日の両方が、DPS にアップロードされる前にポートフォリオ用に設定する必須フィールドでした。 NPR-22484
...
* `cq:cugEnabled` ライブコピー内の別のフィールドが編集されるとプロパティは無効になるので、ページは保護されません。 NPR-22246：CQ-4236050 のホットフィックス
* ページのロールアウトアクションは、ページが中断されたときに子を更新できません。NPR-22483：CQ-4236956 のホットフィックス
* プライマリで移動された構造のロールアウトは問題を引き起こします `cq:moveTarget`. NPR-22373：CQ-4232536 のホットフィックス

### 統合 {#integration-6}

* オファーセレクターライブラリでオファーを並べ替えようとすると、異常な動作が発生します。NPR-22208：CQ-4235439 のホットフィックス
* TargetContentImpl は、長時間実行中のクエリ中にAEMの動作を遅くします。 NPR-22361：CQ-4236907 のホットフィックス
...
* 並べ替え可能なフォルダーの下でフォルダーの並べ替えが保持されない。 NPR-21964：CQ-4231761 のホットフィックス

### Sites {#sites-6}

* （タッチ UI）（クラシック UI）HTL およびコアコンポーネントでの複数のクロスサイトスクリプティング（XSS）脆弱性。NPR-21532：CQ-4232305 および CQ-4232511 のホットフィックス
* 選択したテキストに対してコンテンツの作成や書式設定（新しいリストスタイルの割り当てや削除など）を行っても、Internet Explorer 11 では正常に機能しません。 NPR-21533：CQ-4230689 のホットフィックス
* （Safari）アセットファインダーパネル内のすべてのアセットに表示を設定できません。NPR-21981：CQ-4213720 のホットフィックス
* タイムワープで「RecursionTooDeepException」エラーが返され、ページが文字化けし、日付が変更されても新しいバージョンが作成されない。 NPR-21707：CQ-4199536 のホットフィックス
...
* （Geometrixx ページの）検索抜粋で、停止ワードがハイライトされるのを防ぎます。NPR-21835:Granite-21067 のホットフィックス メモ：この修正には、Oak CFP 1.4.20 以降が必要です。
...
* （iOS11）添付ファイルコンポーネントをクリックすると、デバイスファイルブラウザーではなく、カメラが開きます。 NPR-21926：CQ-4214348 のホットフィックス
* テーマ作成 UI でタイトルが見つからない場合、例外が発生し、ダイアログボックスのレンダリングが失敗します。 CQ-4236143 のホットフィックス
...
* を使用して生成されたPDFファイル `6.2.0-ASM-1017-003` 壊れている。 NPR-21427：CQ-4228046 のホットフィックス
...
* タグピッカーは、複数言語で使用可能なタグを翻訳しません。 NPR-21221：CQ-78855 のホットフィックス
...
* 翻訳ワークフローが 2 文字を超える lang_country コードで失敗する。 NPR-21088：CQ-4197439 のホットフィックス
* 翻訳プロジェクトが完了するまで、翻訳プロジェクトにアセットページを再送信しないでください。 NPR-21219：CQ-4209908 のホットフィックス
...
**注意：** この NPR には、さらに設定が必要です。 参照： [最新の Java™ 8 アップデート](#latest-java-update-throws-an-exception-npr).
...
* インストーラーと LCM で Jsafe Jars を CryptoJ 6.1.3.1 に更新しました。 NPR-21362

#### PDFG サービス {#pdfg-service-2}

* PDFG で Jsafe Jar を CryptoJ 6.1.3.1 に更新します。 NPR-21359
...
* ACL 権限を検出するための検証オプションをパッケージマネージャーで提供します。
...
* docConvertor が、エラー「The prefix &quot;stEvt&quot; for element でPDF/A を生成できない `stEvt:action` バインドされていません」。 NPR-21032：CQ-4222540 のホットフィックス
* 名前を付けて例外がスローされる `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` サービス OMPFSubmission/PDFA/PDFtoPDFA を呼び出す際。 これにより、サーバーが再起動されるまで、短時間のみ有効な署名の検証プロセスが完了しなくなります。NPR-20792
...
* テスト用に onDialogLoaded ユーティリティ関数を追加しました。
...
* ターゲットドロップダウンが、ダイアログボックス内の複数の RTE コンポーネントで機能しない。 NPR-20345：CQ-4220981 のホットフィックス
...
* 日付選択がマルチフィールドのフィールドである場合、日付フィールドに保存された値は、コンポーネントの編集中に保持されません。 NPR-20077：Granite-19147 のホットフィックス
...
* の修正点 `Resourcetype` キャンバスノードの値。
* ドキュメントフラグメントコンテンツで、大文字と小文字を区別する検索機能と特殊文字に対する検索機能を有効にしました。
* Safari でPDFを添付ファイルとして添付するためのアダプティブFormsの機能強化。
...
* 既存のアセットと同じ名前のアセットを同じ場所に移動すると、値は `cq:lastReplicationAction` の場合、アセットが相互にスワップされ、誤ったメタデータが作成されます。 NPR-19531
...
* アダプティブフォームが強化され、Safari で添付ファイルとして PDF を添付できるようになりました。既存のフォームで同じ機能をサポートするには、添付ファイルウィジェットと「サポートされているファイルタイプ」の設定を変更して、.pdf ではなく application/pdf の値をアップデートします。 NPR-19623
...
* シャットダウン中は接続マネージャーを使用できません。JBoss® では、オーサー EAR をデプロイ解除する前に JDBC 依存関係が切断され、破損の問題が発生します。 NPR-19703
...
* ダイアログボックスで複数の RTE コンポーネントを設定します。
* ライブコピーでの参照の更新を強化。
* ドキュメントフラグメントコンテンツで大文字と小文字を区別する検索機能を有効にしました。
* AEM Formsのインストールドキュメントに Linux®® ライブラリのリストを追加しました。
...
* 「アクティベーションのリクエスト」ワークフローをアクティベートすると、AEM インボックスに誤ったオフタイムアイコンが表示される。 NPR-19365：CQ-4216174
...
* `Windows 10` ワークステーションは自動的にタッチモードに切り替わり、一部のボタンが機能しなくなります。 NPR-19183

### Sites {#sites-11}

* ダイアログボックスに複数の RTE コンポーネントを含める場合の問題。 NPR-19311：NPR-19587
...
* Adobe メールテンプレートサービスで、カスタムユーザーテンプレートにタグが追加される。NPR-19190：CQ-4217113 のホットフィックス
...
* ターゲットモジュールを編集して複数回保存すると、ターゲティングモードでターゲティングダイアログボックスが表示されない。 NPR-19144：CQ-4216708 のホットフィックス
...
>* ドキュメントフラグメント内の動的コンテンツ（データディクショナリ要素値や変数値など）に対しては、検索は機能しません。
...
* Microsoft® Windows の管理者画面に、CFP6 をインストールするとバージョン番号 6.0 が表示されます。 CQ-4217573 のホットフィックス
...
* Analytics、DTM および Target のユーザーが設定可能な接続タイムアウト時間が導入され、AEMのパフォーマンスが向上しました。
...
* 別のAEM インスタンスから Parsys コンポーネントにテキストをコピーする場合、Parsys は resourceType が設定されていない状態で作成されます。 NPR-18511：CQ-4212306 のホットフィックス
...
* 翻訳検索結果には、翻訳プロジェクトの名前は含まれません。 NPR-18224：CQ-4210658 のホットフィックス
...
* レターにドキュメントフラグメントを追加するには、長い時間がかかります。 NPR-18958
...
>* 改行のないコンテンツが CM テキストエディターで折り返されない。 NPR-19449
>
...
* PDF Generatorが、指定されたブックマークレベルでPDF文書を生成できない。 CQ-4211102 のホットフィックス
...
* 公開されたビデオは、エンコードされたビデオアセットで再生できません。 NPR-17849：CQ-4210739 のホットフィックス
...
* ページ上の別の画像を削除した後に画像を選択することはできません。 NPR-17711：CQ-4201323 のホットフィックス

### Platform {#platform-11}

* タッチ UI コントロールが、必要な権限を持たないユーザーに対して非表示になりません。NPR-17945：CQ-4211231 のホットフィックス
* タグピッカーフィールドに日本語タグが見つかりません。 NPR-17768：CQ-4210456 のホットフィックス
...
* Apache Sling API で org.apache.sling.servlets.post servlet （2.3.22）のアップグレードバージョンを使用して XSS 脆弱性を回避するリクエスト。 NPR-18963

### 翻訳 {#translation-6}

* 翻訳プロジェクトの場合は、プロジェクトが完了するまでアセットページを送信する必要はありません。 NPR-18249：CQ-4209908 のホットフィックス
...
* 参照されるアセットの表示における「移動」ダイアログボックスの処理能力が向上しました。
* 非公開やバージョンの削除など、WCM のコンポーネントとワークフローに関するいくつかの問題を解決しました。
* ツールバーのアクションと Coral コンポーネントの表示に関するアクションバーの応答性を改善しました。
...
* DAM ワークフローステップでは、Session または ResourceResolver から取得したバイナリストリームは、最終ブロックで閉じる必要があります。 これにより、リソースリークが発生しないようにします。 NPR-17385：CQ-4209452 のホットフィックス
...
* クラシック UI またはタッチ UI を使用してページを移動する場合、移動ダイアログボックスに 150 を超える参照が表示されず、ユーザーはこれらの参照を更新してページを再公開できません。 この問題は、クラシック UI の「maxRefNo」プロパティを導入することで修正されました。これは、siteadmin ノード（/libs/wcm/core/content/siteadmin）で設定可能）です。このプロパティは、大量の移動操作が行われる前に表示される参照の最大数（デフォルト値は 150）を指定します。 ページに複数の参照がある場合、それらの参照は movePage ダイアログボックスには表示されません。 この設定は、それぞれ `'/libs/wcm/core/content/damadmin'` ノードと `'/libs/wcm/core/content/miscadmin'` ノードに設定を適用することにより、damadmin と miscadmin にも適用されます。NPR-17222：CQ-85878 のホットフィックス
...
* Google Chrome ブラウザーでフォームから添付ファイルを削除した後、同じ添付ファイルを再度添付しようとするとエラーがスローされる。 NPR-17297
...
* API メソッド `com.adobe.icc.services.api.LetterInstanceService` の読み込みが機能しない。 NPR-17922、NPR-16008
...
* AEM Forms 6.2 Output サービスを使用して特定のフォームをデータ XML と統合する場合、同じ操作に対して LiveCycle ES4 SP1 サーバーに比べて、20 倍の時間がかかります。Windows および Linux® 環境で修正されています。 NPR-17501
...
* この **診断ツール** AEM 6.1 SP2 からAEM 6.2 SP1 へのインプレースアップグレード後、ページが機能しない。 NPR-17121：CQ-4196786 のホットフィックス
* HTL：フォーラムの選択、トピックの作成および投稿を行う場合 `Sightly SightlyCompiledScript` 不正確な値を追加 `addSelectors` プロパティ先 `RequestDispatcherOption`. NPR-17008：Granite-16384 のホットフィックス
...
* Google Chrome ブラウザーでフルスクリーンモードで表示している場合、ワークフロー通知が遅延読み込みされない。 NPR-17013：CQ-4207567 のホットフィックス
...
* `Camera RAW` アセットにレンディションとして追加したファイル（ARW、CR2、NEF、DNG、EPSを含む）は、選択も削除もできません。 このようなファイルは、ユーザーがクリックすると自動的にダウンロードされます。NPR-16949：CQ-4206846 のホットフィックス
...
* AEM オーサー環境からパブリケーションにプリフライトを実行すると、5 秒以上かかるプリフライトアクションにより、1 秒あたりのステータスリクエスト数が多い AEMM - AEM PECS 統合 Splunk ダッシュボードで異常なスパイクが発生します。 NPR-16908：CQ-4207055 のホットフィックス
...
* 翻訳を使用して作成された言語コピーは、ローカルの国のライブコピーではなくルートプライマリを指します。 NPR-17257：CQ-4208287 のホットフィックス
...
* ネストされた遅延読み込みシナリオがある場合、検証はすべてのパネルに対して正しくトリガーされません。 NPR-15821
...
* ドキュメントフラグメント内の式の先頭または間にタブスペースが含まれるレターは、レンダリングもプレビューもできません。 NPR-16769
...
* 「」をクリック **[!UICONTROL 戻る]** HTMLワークスペース内のフォームの「ボタン」は、フォームを閉じず、グループキューに戻します。
NPR-16352
...
* /apps 下のオーバーレイファイル（JSP または JavaScript ファイル）と/libs 下のホットフィックスに含まれるファイルの競合を検出するためのパッケージマネージャーの検証オプションです。 影響を受けるオーバーレイは、/libs の下のファイルの変更を含むように再ベースできます。NPR-16216：CQ-81729 のホットフィックス
* error.log のログは、パブリッシャーの起動後数秒後に停止することがあり、再実行する場合は消去する必要があります。 ログフレームワークを更新し、Sling ログを提供するリクエスト。NPR-15913：Granite-15452 のホットフィックス
* HTL JavaScript Use API 実装の失敗を回避するために JavaScript &quot; `use"` API を更新するリクエスト。NPR-16461：SLING-6780 のホットフィックス

### Sites {#sites-16}

* AEM 6.0 から AEM 6.2 にアップグレードした後、多数のクエリが原因でタグを検索する際に、クラシック UI のパフォーマンスが遅くなります。この問題を解決するには、[タグ付けコンソール（クラシック UI）のレプリケーションステータスを無効にする](#disable-replication-status-in-tagging-console-classic-ui-npr)に記載されている手順に従います。NPR-15842：CQ-4201748 のホットフィックス。
...
* ターゲティングを使用する場合、キャンペーンを公開しても、ブランドとプライマリを含むツリー全体は自動的には公開されません。 NPR-15855：CQ-94630 のホットフィックス
...
* 添付ファイルウィジェットを使用したアダプティブフォームの場合、添付ファイルのプレビュー後に、フォームの一部が無効になります。
NPR：16611

* 複数の添付ファイルが許可されている添付ファイルウィジェットの場合、以前の添付ファイルを持つウィジェットで添付ファイルを含む新しいフォームインスタンスが送信されると、エラーコードが表示されます。 このエラーは、実際のコンテンツではなく、追加された添付ファイルを開いたときに発生します。 NPR-16258
* `file://`、`http://`、`ftp://` などのプロトコルを使用して、フォームの事前入力サービスを不正アクセスから保護します。参照： [Configuration Manager を使用した事前入力サービスの設定](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15414
...
* Internet Explorer ブラウザーを使用してテキストドキュメントフラグメントで作業している場合、カーソルがテキストインデントを許可しないので、テキストにインデントを設定しようとすると失敗します。 NPR-16128
* 既存のテキストドキュメントフラグメントに対するデータ要素の追加または変更には時間がかかり、ユーザーに通知されない場合があります。 NPR-16102
...
* Google Chrome ブラウザーを使用してテキストドキュメントフラグメントを作成または編集中に、色選択ドロップダウンが自動的にポップアップ表示され、削除できません。フラグメントを編集できるようにするには、ユーザーがデータ入力のタイプとしてリストを選択する必要があります。 NPR-16067
* を使用している間 `Letterinstance` API、メソッド `import com.adobe.icc.services.api.LetterInstanceService` は機能しません。 NPR-16008
* Asset Composer 設定で日付の表示形式を `locale=en_US; dateFormat=MMM dd,yyyy;` に変更すると、期待どおりに動作せず、日付の形式が無効な文字として表示されます。NPR-16007
* 再作成中のレター内のデータリンケージのタイプは、以前と異なる設定の場合でも、「ユーザー」として表示されます。 NPR-16619
...
* 「file://」、「http://」、「ftp://」などのプロトコルを使用して、フォームの事前入力サービスを不正アクセスから保護します。詳しくは、[Configuration Manager を使用した事前入力サービスの設定](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)を参照してください。NPR-15434
...
* 翻訳ワークフローの改善、Azure portal の新しい翻訳 API を使用するための強化されたMicrosoft® コネクタ
* プロジェクトとキャンペーンの修正点
...
* を使用する場合、潜在的な null ポインター例外は回避されません `com.adobe.granite.infocollector.impl.FilesTraversal`関数呼び出し。 NPR-15169：CQ-4197640 のホットフィックス
* 一部の二次ノードではワークフローの状態が一貫しておらず、そのノードの監視イベントをディスパッチ中にエラーが表示されます。NPR-15701：Granite-13786 のホットフィックス
* ユーザーが CRXDE でノード（/content/dam/など）を選択してから「アクセス制御」タブを選択する場合、アクセス制御リストが存在することを確認し、一部の要素をドラッグ&amp;ドロップすると、選択した要素ではなく、要素が移動します。 NPR-15696：Granite-16300 のホットフィックス
...
* 編集中 `TagInput` フィールドコンポーネントに、ダイアログボックスのフィールドに対して以前に設定された値が表示されません。 NPR-15464：CQ-4200360 のホットフィックス
...
* クラシック UI のスライドショーコンポーネントでは、スライドを作成したり、画像をドラッグ&amp;ドロップしてスライドを作成したりすることはできません。 NPR-15281：CQ-4194164 のホットフィックス
* ユーザーには、権限に関係なく、サイトAdmin Consoleに「ページを作成」、「サイトを作成」、「ライブコピーを作成」、「ローンチを作成」、「カタログを作成」などの「作成」オプションが表示されます。 NPR-15278：CQ-94436 のホットフィックス
* AEM 6.2 サービスパック 1 のインストール後、「サブページを含める」スライダーがページ起動時に機能しなくなります。NPR-15230：CQ-4198449 のホットフィックス
* ブロック内のバージョンを取得して処理し、指定されたパスを XPath クエリで使用できるように、バージョンのパージを強化するリクエスト。 NPR-15186：CQ-109205 のホットフィックス
...
* ビデオプロファイルの作成時に高品質と中品質の両方のビデオエンコーディングプリセットを選択した場合、加えられた変更は保存されません。 NPR-15447：CQ-4195482 のホットフィックス
* サーバーエラー応答が原因でBrand Portalへのアセットのアップロードに失敗しても、Brand Portal UI でステータスが「公開済み」に更新され、失われたファイルを追跡するのが困難になります。 NPR-15442：CQ-4197968 のホットフィックス
* アセットフォルダーをBrand Portalに公開する場合（公開に 1 時間以上かかる場合）、一部のファイルが公開に失敗します。 NPR-15441：CQ-4199493 のホットフィックス
...
* プロキシサーバー経由での接続時にYouTubeにビデオを公開する。 NPR-15109：CQ-110332 のホットフィックス
* ドットまたはピリオド（.）を含む名前のアセットを使用する
data-sly-resource は同じアセットに解決されず、出力パスはドットで終了します。NPR-15069：CQ-4195914 のホットフィックス
* AEM 6.2 をサービスパック 1 にアップグレードすると、Scene7へのアセットの同期が失敗します。 この `dam:Scene7FileStatus` プロパティの表示 `UploadFailed` 公開済みアセットの場合でもステータスです。 NPR-15269：CQ-4197708 のホットフィックス
...
* 翻訳プロジェクトを使用する場合、英語のプライマリから言語コピーを更新すると、11 の異なるローンチがすべて同じ名前とソースルートで生成されます。 ただし、ページ名が設定されたパターンに従っている場合は、ローンチルートがそれぞれ若干異なります。 NPR-15605：CQ-4200699 のホットフィックス
* 言語ルートの名前にハイフンやダッシュが含まれている場合、ページの翻訳プロジェクトは作成されません。NPR-15171：CQ-96286 のホットフィックス
* Microsoft® が Azure portal で提供するMicrosoft® Translator API を使用できるように、Microsoft® コネクタを更新するリクエスト。 NPR-15320：CQ-101010 のホットフィックス
...
* キャンペーンの使用中 – ターゲティングと `MAC` - テストおよびターゲット統合コンポーネントで、アクティビティの非公開を行っても、プライマリUI のアクティビティステータスが更新されない。 NPR-15401：CQ-4199839 のホットフィックス
...
* ドキュメントフラグメントの参照の保存、削除および設定には、かなりの時間がかかります。 NPR-15939
...
* 最新バージョンの Java™ 1.8.0_121-b13 にアップグレードすると、AEM Formsで管理者ユーザーインターフェイスにアクセスできなくなります。 NPR-15330
...
* 未保存のタスクを復元する場合、ユーザーエラーを減らすために、未保存のタスクの復元に表示されるメッセージをより明確にする必要があります。 NPR-15377
...
* WCM Sightly Foundation の Parsys コンポーネントでは、コンポーネント宣言タグ名を無視します。 `cq:htmlTag / cq:tagName`. NPR-14225
* Sightly Parsys を使用して、タッチ UI で JavaScript を介して挿入されたコンポーネントをレンダリングすると、ページが更新された後、カスタム装飾は無視されます。 NPR-14122
* リンクなど複数のリッチテキストフィールドが作成される場合、タッチ UI ダイアログボックスで Target ドロップダウンリストが機能しない。 NPR-13911
* ダイアログボックス（タッチ UI）で複数のリッチテキストエディター（RTE）プロパティを持つテキストフィールドを編集すると、フォーカスが特定の RTE プロパティにランダムに移動します。 NPR-13703
...
* エディターからページプロパティ ダイアログボックスを送信すると、変更されていないライブコピータブのデータが書き換えられます。 NPR-14693
...
* 「条件を指定して繰り返し」式を持つアセットが正しく表示されません。ユーザーが以下をプレビューした場合 `*CDN3835RLCEN*` レターテンプレート。本文のターゲット領域にあるアセットは表示されません。 アセットが `*VIPReassement*`（事前に選択されたオプションのアセット）を選択解除すると、事前に選択された他のアセットがレターに表示されます。 NPR-14844
...
* 基になるメタデータスキーマのドロップダウンフィールドで TypeHint のブール値を使用しているときに、アセットのメタデータを一括（追加モード）で編集すると、エラーが発生します。 NPR-14529：CQ-106876 のホットフィックス
...
* でユーザーがすべての選択肢を選択すると、アセットレポートページが応答しなくなります `Configure Columns` ダイアログが表示されます。 NPR-13187：CQ-95589 のホットフィックス
* Safari および Internet Explorer でのタグ選択の予期しない動作。NPR-13134
* アセット管理者の検索レールから保存済みの検索を編集すると、ネストされたスマート選択として保存できるので、環境の安定性の問題が発生します。 NPR-13119：CQ-99460 のホットフィックス
* ファイル（またはフォルダー）を移動してから名前を変更した後、 `cq:name` メタデータに新しいファイル名（フォルダー名）が反映されません。 NPR-13036：CQ-99141 のホットフィックス
...
* Coral-select コンポーネントの使用時（*granite/ui/components/coral/foundation/form/select*）、選択した値に 1 つの項目が含まれる場合、ダイアログボックスで、Internet Explorer （IE11 または Edge ブラウザー）の値の初期化が正しく機能しません。 NPR-13395：CQ-101013 のホットフィックス

### プロジェクト {#projects-5}

* 翻訳方法を「人間」として、翻訳プロバイダーを「なし」として作成した翻訳プロジェクトを書き出す場合、GUID マッピングファイルがないので、translation_export_summary.xml ファイルは生成されません。 NPR-13137：CQ-91976 のホットフィックス
* AEM プロジェクトで「期日」プロパティが設定されたプロジェクトを作成する場合、サーバーとクライアントのタイムゾーンの違いにより、日付の変換で時間が誤って設定されます。 NPR-13003：CQ-98288 のホットフィックス
...
* タッチ UI のインボックスと通知バッジのパフォーマンスが向上しました。 NPR-14145：CQ-101125 のホットフィックス
...
* 多数 `AEM-Scene7` AEMのアクティベーション時に作成およびキャンセルされた sling ジョブは、レプリケーション時にアーカイブジョブとして記録されます。 NPR-12835：CQ-86115 のホットフィックス

### セキュリティ {#security-5}

* WCMDebug フィルターの入力検証の問題を解決するリクエスト。NPR-12444：CQ-94890 のホットフィックス要求
* ローンチ作成ウィザードの使用中に XSS 動作を修正するためのプロアクティブなリクエスト。
...
* Correspondence Management アセット（レター、ドキュメントフラグメント、その他のタイプなど）を検索すると、「ダウンロード用キュー」アイコンがツールバーに表示されません。 NPR-14745
...
* ユーザーインターフェイスから Correspondence Management アセットを一括で書き出すと、AEM Forms Server は不要なログを生成します。 NPR-15226
* レターをプレビューすると、均等配置されたテキストが別のフォントで表示されます。NPR-15468

`**Forms Portal**`

* ポータル送信からの新しいドラフトが送信されると、Forms ポータルで送信済みフォームの添付ファイルが表示されない。 NPR-13515
...
* ユーザーログイン関連の監査イベントが無効な時間で記録される。 監査イベントの正しい時間が追跡できません。NPR-13107
* Adobe Acrobat ReaderとMicrosoft® Office で、拡張認証で保護されたドキュメントを開けない。 NPR-14482
...
* アダプティブフォームのルールビルダーは、役割ベースのアクセスを提供しません。 作成者がおこなった変更が追跡できません。NPR-12840

`Core` (Forms JEE インストーラー):

* サーブレットフィルターとしてのコアクロスオリジンリソース共有（CORS）機能が、JBoss®+で有効になっていません。 NPR-13050
...
CFP パッケージは、ソフトウェア配布から直接ダウンロードするか、次の手順を実行します。
...
>[!NOTE]
>
>アドビが提供するオプションの機能パックは、リリースのバージョンと累積修正パックに依存関係があります。機能パックがインストールされている場合は、 [AEM カスタマーケアチーム](https://experienceleague.adobe.com/ja?support-solution=General#support) AEM 6.2 用のこの累積修正パックとの互換性を検証します。
...
* AEM 6.2 サービスパック 1 が CFP の前提条件です。インストール手順については、リリースノートのを参照してください [AEM 6.2 サービスパック 1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
### ソフトウェア配布を使用して CFP をインストールする {#install-the-cfp-via-package-share}

既存のAEM 6.2 SP1 インスタンスに累積修正パックをインストールするには、次の手順を実行します。

1. パッケージをダウンロードするには、 [ソフトウェア配布](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip).
...
* [パッケージマネージャーから HTTP API](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) を使用します。`cmd=install&recursive=true` を使用して、ネストされたパッケージをインストールします。

### インストールの検証 {#validate-installation}

1. [Product Information] ページ （/system/console/productinfo）の [Installed Products] （インストール済み製品）に、更新後のバージョン文字列「Adobe Experience Manager, Version 6.2.0.SP1-CFP20」が表示されるようになりました。
...
1. [AEM Forms アドオンパッケージのインストール](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)の記載どおりに Forms アドオンパッケージをインストールします。
...
## DTM、Analytics、Target 接続のユーザーが設定できるタイムアウトパラメーター {#user-configurable-timeout-parameters-for-dtm-analytics-target-search-promote-connections}

AEM 累積修正パック 6.2 SP1-CFP7 以降のリリースでは、次の詳細に従って、上記のすべての接続で接続のタイムアウト期間を設定できるようになりました。

| **接続** | **接続タイムアウト&#42;** | **ソケット タイムアウト&#42;&#42;** |
|---|---|---|
| DTM | 30000 ミリ秒 | 30000 ミリ秒 |
| Analytics | 30000 ミリ秒 | 30000 ミリ秒 |
| ターゲット | 60000 ミリ秒 | 30000 ミリ秒 |

* **接続タイムアウト&#42;**  – 接続が確立されるまでのタイムアウト（ミリ秒単位）。 0 のタイムアウト値は、無限タイムアウトと解釈されます。
* **ソケット タイムアウト&#42;&#42;** - データ待機のタイムアウト （ミリ秒）、または 2 つの連続したデータパケットの間に可能な無操作状態の最長時間（ミリ秒）。
...
NPR-21355 は CFP12.1 に含まれています。CFP12.1 以降をインストールしている場合は、以下の手順を実行して JBoss® アプリケーションサーバーに NPR-21355 を設定します。 oracle WebLogic またはIBM® WebSpehere アプリケーションサーバーで動作しているAEM Forms Server に CFP12.1 をインストールする場合、追加の設定は必要ありません。

1. module.xml ファイルをバックアップ、削除、および作成します。 ファイルのデフォルトの場所は [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/ です。
...
1. のバックアップを作成 `jsafeFIPS.jar`, `jsafeJCEFIPS.jar`、および `certjFIPS.jar` のファイル [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/を削除し、前述のディレクトリからファイルを削除します。

連絡先 [Adobeサポート](https://experienceleague.adobe.com/ja?support-solution=General#support) そのため、新しい JAR ファイルを取得できます。 [Adobe サポート](https://experienceleague.adobe.com/ja?support-solution=General#support)から取得した JAR ファイルを [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/ に配置します。
...
* クラスター設定の JBoss® サーバーの場合は、domain.conf.bat を開いて編集します。
  ...
  を設定 `JAVA_OPTS=%JAVA_OPTS%-Djnlp.com.rsa.cryptoj.fips140loader=true`

  を設定 `JAVA_OPTS=%JAVA_OPTS%-Dcom.rsa.cryptoj.fips140initialmode=NON_FIPS140_MODE`
...
ユーザーがタスクを要求しても、共有キューの数は、デフォルトでは他のユーザーについては更新されません。 この目的のために、Adobeは新しいプロパティを導入しました。 次の手順に従って、AEM インスタンスでこのプロパティを設定します。
...
1. これで、システムからログアウトして、もう一度ログインできます。
1. ワークスペース内の他の開始が更新する共有キューユーザーの数です。
1. ポーリングをオフにするには、値を 0 に変更し、XML ファイルを再度読み込みます。

## UI の変更 {#ui-changes}

* を含む画像の画像カードにタイトルを表示する際の動作の変更 `dc:title` プロパティを文字列に設定 [] （マルチフィールド）:UI の画像カードには、最新の変更されたタイトルのみが表示されますが、すべてのタイトルは CRX に保存されます。 CQ-4217165 のホットフィックス
...
* （Adobe Granite メンテナンススケジューラー更新タスク） com.adobe.granite.maintenance.impl.TaskScheduler：ウィンドウに WorkflowPurgeTask という名前のメンテナンスタスクが見つかりません `granite:weekly`
...
* スマートタグ機能パックが含まれているAEM 6.2 SP1 に CFPx をインストールすると、以前にスマートタグアセットに追加したワークフローステップが DAM アセットの更新ワークフローから削除されます。

## Uber Jar {#uber-jar}

6.2 SP1-CFP20 の Uber Jar は、Adobeのパブリック Maven リポジトリーで入手できます。
...
>[!MORELIKETHIS]
>
>* [AEM 6.2 ホットフィックスページ](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.2 SP1 リリースノート](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 6.2 リリースノート](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 製品ページ](https://business.adobe.com/jp/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.2 ドキュメント](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Adobe の優先製品アップデート](https://experienceleague.adobe.com/en/docs/release-notes/experience-cloud/current)
