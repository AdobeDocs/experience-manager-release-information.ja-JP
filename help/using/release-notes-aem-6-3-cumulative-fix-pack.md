...
>[!NOTE]
>
>バージョン 6.3.3.0 より前のAEM インスタンスの場合、Adobeでは、AEM インスタンスにユーザーが多いお客様には、インストールフォルダーを経由して SP/CFP をデプロイすることをお勧めします。
...
* Parsys とその中のコンポーネントを選択し、キーボードショートカットを使用して選択した項目を削除すると、アクションによってコンポーネントとその親 Parsys の両方が削除されます（NPR-32071）。
...
* オーサーインスタンスで変更され、アクティベートされたコンポーネントのコンテンツは、コンポーネントが再起動されるまでパブリッシュインスタンスに表示されません **com.day.cq.personalization.impl.TargetedContentManagerImpl** （NPR-32489 および NPR-32232）。
* 公開時に Context Hub のパフォーマンスがクラッシュする（NPR-31170）。

### Brand Portal {#brand-portal}

* Adobe DeveloperがBrand PortalのAdobe Experience Manager 6.3 と統合されていません（NPR-32056）。
...
* （JEE のみ） Apache に対して報告された重大なセキュリティ脆弱性（CVE-2021-44228 および CVE-2021-45046） `Log4j2`.
...
* プライマリページのページプロパティの読み込みに失敗し、NullPointerException が返される。 この問題は、を追加することで解決されます `cq:blueprin`t プロパティ（NPR-30901）。
* ロールアウト設定が、ルートノード上の blueprintConfig から適切に取得されない。アクティベート解除が、ブループリントとライブコピーの両方に対してトリガーされる。ブループリントのアクティベート解除のみをトリガーにする（NPR-30866）。
...
* 標準のデザインインポーターコンポーネントを含んだAdobe Campaignでキャンペーンを同期できません（NPR-30890）。
* リッチテキストエディター（RTE）で、埋め込みテーブルをリストアイテムとして挿入できません（NPR-30878）。
...
* データ電源を介したAdobeスマートタグ接続の問題。 NPR-30026：CQ-4269457 のホットフィックス
...
* PDFサービスへのHTMLで負荷テストを実行すると、エラーが発生して失敗し、AEM Forms サーバーからファイルタイプの設定が削除されます。 NPR-30111、NPR-30086：CQ-4271495 のホットフィックス
...
* DAM CQ Mime タイプサービスが m3u8 にテキストを適用すると、アセットが使用できなくなる。 NPR-29259：CQ-4264052 のホットフィックス
...
* ロールアウト時に空のプロパティと複数のプロパティがブループリントから反映されない。ブループリントを使用したライブコピーのリセットは、コンポーネントには機能しません。 NPR-29252：CQ-4264928、CQ-4264926、CQ-4267722 のホットフィックス
...
* 同じ名前のアーカイブを再アップロードすると、新しく処理されたアセットに対してレンディションが生成されません。 NPR-28643：CQ-4262286 のホットフィックス
...
* 大きな zip アーカイブアセットをアップロードすると、CommitFailedException トリガーが発生する。 NPR-28528：CQ-4260903 のホットフィックス
...
* ネストされたレイアウトコンテナとテキストコンポーネントで Target コンポーネントを使用すると、テキストの編集時またはコンテナのクリック時に、JavaScript エラー「Cannot read property currentPos of null」がスローされる。 NPR-29077：CQ-4246594 のホットフィックス
...
* プライマリで移動された構造のロールアウトは問題を引き起こします `cq:moveTarget`. NPR-28968：CQ-4265280 のホットフィックス

### 統合 {#integration}

* （Cloud Service設定）ルートレベルに表示される「継承元」チェックボックスを削除する必要があります。 NPR-28771：CQ-4259676 のホットフィックス
...
* Bright Edge 資格情報を使用すると、接続エラーで失敗する。 NPR-29167：CQ-4265872 のホットフィックス
...
* タグをクリックすると、グローバルなマウスアップイベントが削除され、ダイアログボックスが「ドラッグ可能モード」でフリーズする。 NPR-28641：CUI-7294 のホットフィックス
...
* Shared queue response task は、HTML5 ワークスペースでフラッシュ要素を開きます。 NPR-29161：CQ-4266498 のホットフィックス
...
* 組み込み型のリポジトリ（Apache Jackrabbit Oak）をバージョン 1.6.16 に更新しました。
* 1 ページあたり 50 レコードに制限するようにポリシーセットリストのページネーションを変更しました。
* パブリッシュインスタンスの AEM Communities User Sync Listener で、無視されるノードに rep: cache を追加しました。
* リストおよびカード表示ボタンに aria-label を追加しました。
* 検索の実行時に、コンマにエスケープ文字が含まれていました。
* コンテンツポリシーの合成リソースのサポートを有効にしました。

#### Assets {#assets-5}

* タイプが複数のファイルをダウンロードできません `.jp2`, `.max`, `.oft`, `.msg`. NPR-28002：CQ-4210856 のホットフィックス
...
* のような文字の検索中にエラーが発生しました `(` , `+` , `?` （クイック検索）。 NPR-28212：CQ-4260969 のホットフィックス
* API を使用してユーザー生成コンテンツのコメントを削除できない。 NPR-28075：CQ-4260534 のホットフィックス
...
* 親コンポーネントのを設定せずに、ネストされた Parsys にテンプレートポリシーを設定できません。 NPR-27987：CQ-4246095 のホットフィックス
...
* ユーザーが注釈をクリックすると、注釈のハイライトが消える。 BPR-27196：CQ-4254423 のホットフィックス
...
* Admin Consoleの [Policy Sets] ビューに表示されている最初の 1000 を超えるポリシー・セットが表示されない。 NPR-28060、NPR-26047：CQ-4249865 のホットフィックス
* 名前を付けて例外がスローされる `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` 短時間のみ有効なプロセスを完了させない。 NPR-28652
...
AEM累積修正パック 6.3.3.2 は、AEM 6.3 サービスパック 3 に依存しています。 したがって、AEM 6.3 サービスパック 3 をインストールした後、AEM 累積修正パック 6.3.3.x パッケージをインストールする必要があります。インストール手順については、AEM 6.3 サービスパック 3 のリリースノートを参照してください。
...
* 公開のグループリストページのページネーションサポートを有効にしました。
...
* （外部リンクチェッカー）内部リンクは、個々のページで壊れているように表示されますが、外部リンクでは機能しません。 NPR-27481：CQ-4257780 のホットフィックス
...
* この `cq:actions` は、ターゲットコンポーネントには考慮されません。 NPR-27616：CQ-4257497 のホットフィックス
...
* 翻訳のプレビューは、we.retail サンプルコンテンツでは機能しません。 NPR-27170：CQ-4241179 のホットフィックス

* platform.login の事前対応的な修正。NPR-26961
* Tomcat を使用したAEM WAR で、ページプロパティの「保存」および「閉じる」が適切なページに戻らない。 NPR-27567：Granite-23671 のホットフィックス

* 保存すると、入力したテキストは sourceEdit 機能によって失われます。 CQ-4259273 のホットフィックス
...
* 編集ダイアログボックスのコンテンツポリシーに応じて、リッチテキストエディターのツールバー項目を有効/無効にできます。
...
* アセットに多くの関係を作成すると、エラーが発生する。 NPR-26309：CQ-4250708 のホットフィックス
...
* granite coralui2 および coralui3 clientlibs を更新して、ビルドからハンドルバーを削除します。 NPR-25606：Granite-22116 のホットフィックス
...
* expressionResolver が null の場合、レンダリング条件が正常な結果を返すことを確認しました。
...
* ファイル拡張子 LSR および SKETCH の抽出された MIME タイプが正しくないので、無効なファイルダウンロードが発生します。 NPR-23644：CQ-4243260 のホットフィックス
...
* Sling ダイナミックインクルードを有効にした Dispatcher を介して Communities コンポーネントにアクセスする場合の問題。 NPR-23586：CQ-4242360、CQ-4241522 のホットフィックス
...
* Designer と Output サービスを使用して生成された XML フォームにPDF/UA のサポートを追加。 NPR-23132
...
* ワークフローインボックスの新しい項目に、ページのタイトルではなくページパスが表示される。 NPR-21634：CQ-4230672 のホットフィックス
...
* （クラシック UI）リッチテキストエディターの出力 HTML で、「list-position-style: inside;」がインラインスタイルとしてに追加される &lt;ul> タグ。 NPR-22145：CRTE-114 のホットフィックス
...
* 最初のコンテンツルートで「サブページを含める」がオンになっていない場合、複数のページを含んだローンチでサブページが公開サーバーに公開されない。 NPR-22482：CQ-4237818 のホットフィックス
* （タッチ UI）クラシック UI コンソールを介してローンチを削除すると、すべてのページが編集できなくなります。 NPR-22491：CQ-4225074 のホットフィックス
* ダイアログに余分なスペースがあるため、画像コンポーネントに問題が発生します。 NPR-22528：CQ-4238183 のホットフィックス
...
* パブリッシュインスタンスをオフにしたページをアクティブ化すると、はオレンジに変わるのではなく、グリーンのステータスになります。 NPR-22927：CQ-4236310 のホットフィックス
* （StyleSystem）ポップアップからスタイルを選択すると、画面の位置がジャンプする。 NPR-23183：CQ-4238867 のホットフィックス
...
* ルールエディターのカスタム関数に関数の呼び出し後に追加が追加されているため、カスタム関数が true を返しても検証に失敗します。 NPR-22481：CQ-4235499 のホットフィックス
...
* PDF/A-1b コンプライアンスチェックエラーに関するAcrobat DCとAEMのレポートの不一致。 NPR-22051、NPR-22050：CQ-4226128、CQ-4227671 のホットフィックス
...
* HSM を使用してPDFドキュメントのデジタル署名/認証を試行中に例外が発生する。 NPR-21154：CQ-4226978 のホットフィックス
...
* dc: title と dc: description がCRXDE Lite内の複数フィールド値に変わらない。 NPR-19570：CQ-4209086 のホットフィックス
...
* インスタンスをAEM 6.1SP2-CFP3 にアップグレードした後、Sidekick.js のコード変更が原因で、マルチコンポジットフィールドのドロップダウン値が再入力されない。 NPR-19450：CQ-4194771 のホットフィックス
...
* JBoss® アプリケーションのクラスター設定で動作しているAEM Forms Server が停止すると、アプリケーションサーバーがデータベースから切断されます。 データ破損の問題につながる可能性がある。NPR-19724
...
* 重複するアセットがアップロードされると、検出された重複項目ダイアログボックスで選択されていない場合でも、すべてのアセットに「削除」または「保持」オプションが適用されます。 CQ-4201673
...
* JBoss® EAP 6.4 にデプロイされたAEM 6.3 のタッチ UI でのページ作成の問題。NPR-17589:CQ-4210137 のホットフィックス
...
* ブランド/ゾーンをサイトのルートページに設定すると、サブページ内の領域のキャンセルされた継承を復元できない。 NPR-17753：CQ-4210139 のホットフィックス

### ワークフロー {#workflow-3}

* 一時的でないワークフローで、外部プロセスステップの前に行われたプロセス履歴とメタデータの変更が保持されない。NPR-17848：Granite-17757 のホットフィックス
* ワークフローダイアログボックスのフィールドの値が、作業項目のノードに保持されない。 NPR-17734：CQ-4210369 のホットフィックス
...
* PDF/A-1b 準拠のPDFドキュメントを検証すると、AEM Formsが検証エラー「Form field has multiple appearances. The PDF documents are PDF/A-1b compliant.」を返す。NPR-18013
...
* Microsoft® Excel およびMicrosoft® PowerPoint では、AEM Document Security Extension for Microsoft® Office で保護されたドキュメントを開くのに時間がかかります。 CQ-4212358
...
AEM累積修正パック 6.3.0.1 は、2017 年 4 月のAEM 6.3 の一般リリース以降に対応された、内部およびお客様向けの修正を含む重要な更新です。 AEM 累積修正パックの主なハイライトは次のとおりです。
...
activity.log が crx -quickstart /logs フォルダーに作成されます。
...
* adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml を開いて、次のように編集します。
   ...
   * &lt;init-param>
&lt;param-name>sling.bootdelegation.ibm&lt;/param-name> &lt;param-value>com.ibm.xml です。&#42;,com.ibm.crypto.pkcs11impl.provider,com.ibm.pkcs11,com.ibm.pkcs11.nat&lt;/param-value>
&lt;/init-param>

* WebSphere® アプリケーションサーバーから以前のエンタープライズアーカイブ（EAR）ファイルをアンインストールし、のセクション 10.2 の手順に従って、更新された EAR ファイルをインストールします。 [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/jp/pdf/aem-forms/6-3/install-single-server-websphere.pdf)
...
>[!NOTE]
>
>この設定は、6.3.2.2 以降に適用されます。「merge_preserve」 acHandling が原因で ACL が CFP インストールを通じて更新されないので、CRX-DE を使用してアクセス制御リスト（ACL）ポリシーを手動で更新するように指示します。
