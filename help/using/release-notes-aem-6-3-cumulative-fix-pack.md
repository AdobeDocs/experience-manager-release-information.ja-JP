...
* 名前にパーセント記号（％）が入ったフォルダーを含むアーカイブを解凍すると、Assets のインターフェイスで開けません。NPR-29989：CQ-4270467 のホットフィックス
...
* HTML を PDF に変換するサービスで負荷テストを実行すると、エラーが発生して失敗し、AEM Forms サーバーからファイルタイプの設定が削除されます。NPR-30111、NPR-30086：CQ-4271495 のホットフィックス
...
* プライマリリードで移動された構造をロールアウトすると、間違った `cq:moveTarget` が発生する。NPR-28968：CQ-4265280 のホットフィックス
...
* タグをクリックすると、グローバル mouseup イベントが削除され、ダイアログボックスが「ドラッグ可能モード」でフリーズする。NPR-28641：CUI-7294 のホットフィックス
...
* 基盤フォームと共にコアコンポーネントフォームを使用すると、null ポインター例外が発生する。NPR-27333：CQ-4249176 のホットフィックス
...
* AEM Sites ページで間違ったフォームが設定され、「フォームがページの幅全体をカバー」チェックボックスが選択されると、AEM Forms コンテナコンポーネントが非表示になる。NPR-25972：CQ-4239287、CQ-4249133 のホットフィックス
...
* ページプロパティのサムネールタブが、ダイアログボックスを拡張すると機能しない。NPR-22844：CQ-4241474 のホットフィックス
...
* DatePicker が、非表示フィールドによって設定された外部タイプヒントの手動設定をサポートしていません。タイプヒントを変更すると、変換エラーが発生します。NPR-23371：Granite-21194 のホットフィックス
...
* パブリッシュインスタンスをオフにしてページをアクティブ化すると、アンバー色ではなく緑色のステータスになる。NPR-22927：CQ-4236310 のホットフィックス
...
* カスタムの at.js または mbox.js を指定すると、インクルードされたスクリプトが、HTML タグではなくテキストとしてページに書き込まれます。NPR-22441：CQ-4203691 のホットフィックス
* ターゲットモードで、作成者がブループリントから継承されたコンポーネントを、継承をキャンセルせずに変更できます。NPR-22751：CQ-4237907 のホットフィックス
* `jcr:content` ノードが見つからないので、PersonalizationDataSource で null ポインター例外が発生します。NPR-22850：CQ-4222122 のホットフィックス
* 英語以外の言語を使用すると、AEM のターゲティングが失敗します。NPR-22917：CQ-4218213 のホットフィックス
...
* ルールエディターのカスタム関数が余分な情報を追加するので、関数の呼び出し後、カスタム関数が true を返しても検証に失敗する。NPR-22481：CQ-4235499 のホットフィックス
...
* ProductAssetsUploader によって Java™ TEMP フォルダーに Java™ キャッシュの一時ファイルが残されます。NPR-20256：CQ-4221801 のホットフィックス
...
* オムニサーチの文字が多すぎると、AEM サーバーがクラッシュするおそれがあります。NPR-20434：CQ-4223602 のホットフィックス
...
* 一部のカスタムデプロイメントで、タスクの追加モジュールで担当者を選択しようとすると、ユーザーピッカーでリストを作成するのに時間がかかります。NPR-20283：CQ-4224193 のホットフィックス

### ユーザーインターフェイス {#user-interface-3}

* ダイアログボックスの属性にかかわらず、カラーフィールドが「常に必須」に設定されます。NPR-19702
...
* HTML Workspace でデータを送信した後、複数行テキストフィールド（TextArea）にテキスト中の改行文が保持されない。NPR-20085
...
* 重複するアセットがアップロードされると、検出された重複項目ダイアログボックスで選択されていない場合でも、すべてのアセットに「削除」または「保持」オプションが適用される。CQ-4201673
...
デフォルト値は 1 です。ファイル constants.js（`/libs/cq/ui/widgets/source/constants.js`）をオーバーレイして、CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL プロパティのコメントを外し、必要な値（ページプロパティのデータが保存される、ページの `/jcr:content` 配下の最大階層数）を割り当てることで、値をオーバーライドします。
