...
* マスターページのページプロパティの読み込みに失敗し、ヌルポインター例外が返される。この問題は、cq:blueprint プロパティを追加することで解決される。（NPR-30901）
* ロールアウト設定が、ルートノード上の blueprintConfig から適切に取得されない。アクティベート解除が、ブループリントとライブコピーの両方に対してトリガーされる。アクティベート解除は、ブループリントに対してのみトリガーされる必要がある。（NPR-30866）
* ユーザーがページをロールアウトすると、重複したライブコピーパスがロールアウト設定ダイアログに表示される。（NPR-30438）
* デフォルトでは、基礎モードのリッチテキストエディター（RTE）が予期せず、インラインフォントサイズを要素に適用する。（NPR-31283、NPR-30922）
* そのまま使用できるデザインインポーターコンポーネントを含んだ Adobe Campaign でキャンペーンを同期できない。（NPR-30890）
* リッチテキストエディター（RTE）で埋め込みテーブルをリスト項目として挿入できない。（NPR-30878）
...
* Dynamic Video によるビデオ集計で、結果セットの上位 100 項目のみが返される。NPR-30441：CQ-4213561 のホットフィックス
...
* resourceType リクエストパラメーターの値が、二重引用符で囲まれた HTML タグ属性の値にコピーされる。NPR-29832：CQ-4255365 のホットフィックス
...
* HTML PDF 変換サービスで負荷テストを実行すると、エラーが発生して失敗し、AEM Forms サーバーからファイルタイプの設定が削除される。NPR-30111、NPR-30086：CQ-4271495 のホットフィックス
...
* 2 GB を超える Adobe Illustrator アートワークファイルをアップロードすると、Java ヒープ領域エラーが発生する。NPR-29265：CQ-4226217 のホットフィックス
...
* ロールアウト時に空のプロパティと複数のプロパティがブループリントから反映されない。ブループリントの状態へのライブコピーのリセットがコンポーネントに対して機能しない。NPR-29252：CQ-4264928、CQ-4264926、CQ-4267722 のホットフィックス
...
* スマート画像が使用されている場合、エクスペリエンスフラグメントを Target に書き出せない。CQ-4269606 のホットフィックス
...
* ファイル「coral.js」に、ライブラリ「handlebars.js」の脆弱なバージョンが含まれている。NPR-26973：CQ-4255377 のホットフィックス
...
* ポリシーセット表示ページで、ページごとに項目を選択するオプションが有効になりました。
...
* 共有キュー応答タスクにより、HTML5 Workspace で Flash 要素が開かれる。NPR-29161：CQ-4266498 のホットフィックス
* ボタンにウムラウト文字が含まれている場合、Workspace から送信できない。NPR-29014：CQ-4263172 のホットフィックス

#### Forms - Document Security {#forms-document-security}

* ポリシーセット表示ページで、ページごとに項目を選択するオプションを有効にする。NPR-29243：CQ-4268567 および CQ-4265132 のホットフィックス
...
* Designer で XDP を PDF としてプレビューする場合と、同じ XDP を HTML としてプレビューする場合で動作が異なる。NPR-28602：CQ-4260239 のホットフィックス
...
* リッチテキストエディターのフルスクリーンモードで、スタイルプラグインがサポートされていない。NPR-27622：CQ-4258674 のホットフィックス
...
* コンポーネントブラウザーでユーザー入力がサニタイズされないので、JavaScript エラーがスローされる可能性がある。NPR-27986：CQ-4247590 のホットフィックス
* ユーザーがコンテンツフラグメントを編集しようとすると、空白ページが表示される。NPR-27669
* 注釈をクリックするとすぐに、注釈のハイライトが消える。BPR-27196：CQ-4254423 のホットフィックス

#### 統合 {#integration-1}

* ResourceResolver でターゲットコンポーネントの複数のドメインが解決されない。NPR-28265：CQ-107847 のホットフィックス
* アクションが表示されないので、ライブコピーの継承キャンセルがターゲットコンテナで正しく機能しない。NPR-28129：CQ-4259813 のホットフィックス
...
* フォルダーメタデータスキーマで「ルール」タブをサポートし、アセットフォルダーに適用するようになりました。
...
* カスケードドロップダウンが、動的ドロップダウンリストでサポートされていない。NPR-27044：CQ-4252564 のホットフィックス
* ExpiryNotification 機能を使用するようにクエリを改善した。NPR-26999：CQ-4251188 のホットフィックス
...
* リンク共有メール経由でダウンロードすると、空の zip ファイルまたは破損した zip ファイルが返される。NPR-27997：CQ-4259686 のホットフィックス
...
* SP3 をインストールした後、フォルダーメタデータスキーマ UI が機能しなくなる。CQ-4257737 のホットフィックス
...
* UI でイネーブルメント SCORM リソースの完了ステータスラベルの表示が空になる。NPR-27152：CQ-4249994 のホットフィックス
...
* ダイアログモードの RTE に編集設定用の設定サーブレットを追加しました。
* 複合フィールドをサポートするようになりました。
* 編集ダイアログのコンテンツポリシーに応じて、リッチテキストエディターのツールバー項目を有効／無効にできるようになりました。
...
* 編集ダイアログのリッチテキストエディター機能が、ポリシーで制御されない。NPR-27059、NPR-26750：CQ-4241130 のホットフィックス
...
* 「特殊文字」を追加すると、リッチテキストエディターダイアログが上部までスクロールする。NPR-26435：CQ-4249869 のホットフィックス
...
* レプリケーションで OAuth の期限切れトークンが使用されて、複数のエラーが発生する。NPR-25894：Granite-22388 のホットフィックス
...
* ファイル「coral.js」に、ライブラリ「handlebars.js」の脆弱なバージョンが含まれている。CQ-4255377 のホットフィックス
...
* フォーラム投稿の作成時に、添付画像にサムネールが表示されない。NPR-26380：CQ-4253105 のホットフィックス
* Word プラグインからペーストすると、複数の画像を読み込めない。NPR-26728：CQ-4253638 のホットフィックス
...
* 非アクティブキーのサーバーログでプリンシパルキーのロールオーバーの例外が発生する。NPR-26748：CQ-4253705 のホットフィックス
...
* 編集可能なモジュールを編集および保存してから PDF プレビューを開くまたは閉じると、レターの PDF および HTML プレビューが同じウィンドウに表示され機能する。NPR-26770：CQ-4253217 のホットフィックス
...
* HTML Workspace の環境設定画面で変更内容を保存すると、例外がスローされる。CQ-4241485 のホットフィックス
* 最新の JEE セットアップでは一括承認が機能しない。CQ-4241486 のホットフィックス
* 最新の 6.4 JEE サーバーでタスク／開始ポイントのドラフトが失敗する。CQ-4241484 のホットフィックス
* Date.toString() ではなく、Date().getTime().toString パラメーターを設定してセッションを初期化する。CQ-4241483 のホットフィックス
...
* WAS9 で esapi.properties をクラスローダーリソースとして読み込めない。CQ-4255631 のホットフィックス
...
* ダウンロードサイズと表示される情報が一致せず、ユーザーの混乱を招く。NPR-23418：CQ-4242774 のホットフィックス
...
* Dynamic Media ビデオエンコードの問題 - 変更／再アップロードランチャー。NPR-23237：CQ-4240260 のホットフィックス
...
* 複数のローンチを実行すると、ローンチはオーサーで昇格するが、レプリケーション権限がないため、パブリッシュサーバーで変更内容がレプリケートされない。NPR-22934：CQ-4234746 のホットフィックス
...
* Assets のフォルダーに約 80 個のフラグメントが追加されると、タイムラインコンソールからワークフローをトリガーしたときにエラーが発生する。NPR-23393：CQ-4211216 のホットフィックス
...
* モバイル／タブレットバージョンで必要な選択ドロップダウンを含んだダイアログを保存できない。NPR-23904：CQ-4243096 のホットフィックス
...
* 空の値を持つアイテムが選択されている場合、Select RT が検証を行わない。NPR-23405：Granite-21283 のホットフィックス
...
* Sling Dynamic Include を有効にした Dispatcher を介して Communities コンポーネントにアクセスする場合の問題。NPR-23586：CQ-4242360、CQ-4241522 のホットフィックス
...
* ダイアログの FileUpload フィールドが必須なため、ダイアログ送信で問題が発生する。NPR-22464：CQ-4222192 のホットフィックス
...
* Lucene 検索で返される抜粋のハイライト表示に一貫性がない。NPR-22879：CQ-4238513 のホットフィックス
...
* 起動時にスケジューラのデッドロックが発生する。NPR-22440：Sling-7004 のホットフィックス
...
* com.day.cq.analytics.sitecatalyst でリソースリゾルバーが閉じられない。NPR-22323：CQ-4236515 のホットフィックス
...
* 「最小」および「最大」の日付範囲が設定されている場合、Coral カレンダーの月選択に問題がある。NPR-22716：CUI-7187 のホットフィックス
* （クラシック UI）コンポーネントが、関連するフォームデータモデルサービスが空のフィールドに設定されている場合でも、デフォルト値を表示します。NPR-22272：Granite-19744 のホットフィックス

### Granite {#granite-2}

* アセット共有 AEM パブリッシャーインスタンスで、メモリリークに起因する安定性の問題が発生します。NPR-22205、NPR-23178：Sling-5668、Sling-7292、Sling-7470 のホットフィックス。
...
* エクスペリエンスフラグメントメニューでのワークフロー作成を有効にする。NPR-22347：CQ-4221661 のホットフィックス
* 再現性のあるエクスペリエンスフラグメントエラーが WeRetail で発生する。NPR-21958：CQ-4220061 のホットフィックス
* 削除されたエクスペリエンスフラグメントを含んだページをアクティベートすると、NullPointerException がスローされる。NPR-23179：CQ-4239939 のホットフィックス
...
* HTML5 入力タイプのサポートを追加。
* 基本 SOAP ヘッダー認証を修正。
* ハイパーリンクの title 属性のサポートを追加。
* Forms Designer のアクセシビリティツリーで PDF/UA 識別子を有効化。
* Workbench ユーザーの証明書認証を有効化。
* PDF/A-2a または PDF/A-3a に準拠する PDF ファイルの生成のサポートを追加。

### Forms アドオンパッケージ {#forms-add-on-package-9}

#### Forms - 管理 UI {#forms-admin-ui}

* ECM コネクタ設定ページのパスワードフィールドを調べると、パスワードがプレーンテキストで表示される。NPR-22508：CQ-4236065 のホットフィックス
...
* 選択された日付パターンに関係なく、日付選択コンポーネントが、最小および最大の検証メッセージを表示する際にパターンに従わない。NPR-22444：CQ-4236269 のホットフィックス
* 送信リクエストで送信される日付の形式を、日付選択コンポーネントで指定されるパターンに合わせる必要がある。NPR-22384
...
* Workbench を使用したサーバーログのダウンロードが一方のサーバーでのみ機能し、もう一方のサーバーで機能しない。NPR-21079：CQ-4229842 のホットフィックス
...
* HTML フォームに入力された資格情報をブラウザーで保存できない。NPR-21762：CQ-4206543 のホットフィックス
...
* Designer から PDF 形式で保存すると、PDF/UA 識別子がなくなる。NPR-23137
* パスオブジェクト（例：ボーダー、アンダーライン、ハイパーリンク）が、Designer から PDF 形式で保存するとタグ付けされない。NPR-23136
* Designer から PDF 形式で保存すると、画像オブジェクトにバウンディングボックスがなくなる。NPR-23134
* Designer で定義されたインラインハイパーリンクが、Designer から PDF 形式で保存されたときにタグ付けされず、代替テキストもない。NPR-23133
* AEM 6.3 Forms Designer で XML データパッケージを開くと、XML ソースから XHTML 書式が削除される。NPR-21178：LC-3917046 のホットフィックス
...
* HSM を使用して PDF ドキュメントのデジタル署名／認証を試行中に例外が発生する。NPR-21154：CQ-4226978 のホットフィックス
...
* AEM Assets での CUG 機能の実装をサポートするために UI を変更しました。
* エクスペリエンス向上のために、ライセンスチェックページの UI の問題を修正しました。
* AEM Forms アプリで OSGi ワークフロータスクのサポートを有効にしました。

### Assets {#assets-10}

* AEM Assets での CUG 機能の実装をサポートするために UI を変更しました。NPR-19485
...
* Assets API に関する複数フォルダーの処理の問題を解決。NPR-20569
...
* 変更が失敗しエラーがスローされた後にセグメントを再アクティベート。NPR-20054：CQ-4218401 のホットフィックス
...
* ルールエディターでブーリアン制約を使用して既存のルールを編集できない。NPR-21128
...
* 時間のサーバーサイドレンダリングを、基盤時間コンポーネントに置き換えました。
* タグ管理／タグ付けコンソールで AEM から Brand Portal にタグを公開できるようになりました。
...
* オーサーモードでビデオ再生エクスペリエンスをテストする場合、Dynamic Viewer が最低品質のビデオレンディションを読み込む。NPR-19004
...
* 列表示でアセットを選択して「編集」をクリックすると、ページがエラーを返す。NPR-20301：CQ-4224052 のホットフィックス
* 正のタイムゾーンと負のタイムゾーンをまたぐ際に、アセットの有効期限の表示がオフになる。NPR-20329：CQ-4219333 のホットフィックス
...
* インスタンスを AEM 6.1SP2-CFP3 にアップグレードした後、Sidekick.js のコード変更が原因で、マルチ複合フィールドのドロップダウン値が再入力されない。NPR-19450：CQ-4194771 のホットフィックス
* WCMMode.EDIT が、オーサリングモードのターゲットコンポーネントで機能しない。NPR-19387
* コンテンツフラグメントを消費する JSON API を公開。NPR-19500
* 太字、斜体およびアンダーライン機能が、オーサーダイアログのリッチテキストエディターフィールドで機能しない。NPR-19670：NPR-19718：CQ-4219088 のホットフィックス
...
* メール通知が、公開されたコンポーネントでのモデレーター関連のアクティビティで機能しない。NPR-19767：CQ-4218060 のホットフィックス
...
* Google Chrome ブラウザーを使用する場合、タッチ UI で列表示を使用してページ／アセットの完全なリストを読み込むことができない。NPR-19566：CQ-4214248 のホットフィックス

### Brand Portal {#brand-portal-1}

* コメントおよび注釈付きのアセットを AEM から公開できない。NPR-19590：CQ-4218386 のホットフィックス
...
* 検索フォームのレプリケーションが失敗する。CQ-4220080 のホットフィックス
...
* Word ファイルのコンテンツをコピー＆ペーストすると、テキストエディターでコンテンツのカラーとフォントが失われる。NPR-19521

#### Assembler サービス {#assembler-services}

* PDFA-1b 形式に変換するドキュメントのコンプライアンス検証中に、Acrobat と AEM の結果に不一致が生じる。NPR-19280

#### ワークフロー {#workflow-2}

* ワークフロー署名ステップで、HTTP 呼び出しがプロキシを介して接続できるようにする。NPR-20529
...
* ヌルポインター例外のために、プロセスレポートでデータが適切に取得されない。NPR-19759

>[!NOTE]
>
>問題を修正するために、[AEM Forms リリース](aem-forms-releases.md)の記事にリストされている LiveCycle 埋め込みパッケージをインストール。

#### 標準サービス {#standard-services}

* docConvertor が PDF の透明部分の分割・統合をサポートしておらず、PDF/A を生成できない。NPR-16228：CQ-4214488 のホットフィックス

#### コア {#core-2}

* JBoss アプリケーションのクラスター設定で動作している AEM Forms サーバーが停止すると、アプリケーションサーバーがデータベースから切断され、データ破損の問題につながる可能性がある。NPR-19724

### 含まれている機能パック {#feature-packs-included-1}

* アセットの必須フィールド検証が欠けているので、ドロップダウンメタデータスキーマフィールドを必須にできない。NPR-17882：CQ-4208373 のホットフィックス
...
* ライブコピーにおけるコンポーネントレベルの同期の問題を解決
* コンテンツが多いページの列表示での使いやすさを向上
* タイトルが長いページのページ命名規則への準拠を強化
* タッチ UI でのキャンペーンパーソナライゼーションエクスペリエンスを向上
...
* AEM におけるユーザー権限変更の可監査性の必要性。NPR-17061
* DM クラウドサービスへの接続が残るため、「Too many open files」例外が発生する。CQ-4211407

### Assets {#assets-12}

* 様々なオプションを使用したスマートコンテンツサービスの設定に関するユーザビリティの問題。NPR-18200：CQ-4201557 のホットフィックス
* S3 データストアへのバイナリストリームでリソースリークが発生。NPR-18041：CQ-4209506 のホットフィックス
* ASCII／UTF-8 エンコードされたテキストファイルが AEM Assets にアップロードされ、サムネールの生成に失敗すると、エラーが発生します。NPR-18006：CQ-4209345 の CFP
* デフォルトのメタデータスキーマが原因で、コンテンツフラグメントの検証が失敗する。NPR-17769：CQ-4211111 のホットフィックス
* com.day.cq.dam.s7dam.common.analytics.impl.SiteCatalystReportRunner のリソースリゾルバーが閉じられていない。NPR-17598：CQ-4209018 の CFP
* Brand Portal にアセットを公開するために複数のレプリケーションエージェントの作成を要求。NPR-17189
* 日本語フォルダー配下のアセットのレビュータスクが機能しない。CQ-4204782
...
* ハイブリッドデバイス上の Chrome での編集時にメタデータスキーマフォームに新しいタブを追加する場合の問題。CQ-4201810
* 重複するアセットがアップロードされると、検出された重複項目ダイアログで選択されていない場合でも、すべてのアセットに「削除」または「保持」オプションが適用される。CQ-4201673
* 150 を超える参照先を持つアセットフォルダーを移動すると、ヌルポインター例外が発生します。CQ-4200981
* ダウンロードしたアセットフォルダーの場合、ZIP ファイルの内容を抽出するときに競合が発生すると、「両方を保持」ではなく「バージョンを作成」がデフォルトオプションとして表示される。CQ-97800
...
* タッチ UI でコンテンツが多いページを列表示でナビゲートする場合の問題。NPR-17799：CQ-4199611 のホットフィックス
...
* ネストされたグループが存在する場合、ワークフローステータスプロバイダーが原因で、インスタンスがロックされる。NPR-17556：CQ-4202056 の CFP リクエスト
...
* Classic UI を使用したキャンペーンおよび関連エクスペリエンスのパーソナライズに関する問題。NPR-18127：CQ-4211559 のホットフィックス
* ブランド／ゾーンをサイトのルートページに設定すると、サブページ内のエリアのキャンセルされた継承を復元できない。NPR-17753：CQ-4210139 のホットフィックス

### ワークフロー {#workflow-3}

* 一時的でないワークフローで、外部プロセスステップの前に行われたプロセス履歴とメタデータの変更が保持されない。NPR-17848：Granite-17757 のホットフィックス
* ワークフローダイアログフィールドの値が作業項目ノードに保持されない。NPR-17734：CQ-4210369 のホットフィックス
* インボックスからタスクを編集すると、解析不能な日付のエラーが発生する。CQ-4208749

### プロジェクト {#projects-3}

* 様々なプロジェクトオーバーレイの問題の修正。NPR-17733
* プロジェクトモジュール内のポッドを再構築すると、設定しにくくなる。CQ-4209859
* ページが翻訳ジョブに追加されると、アセットのパスが、ローカライズされたそれぞれのサイトに変更される。CQ-4206007
...
* テキストエディターでは箇条書きリストの上下に追加のマージンが表示されるが、HTML および PDF レンディションでは表示されない。NPR-18126
...
* PDF ドキュメントが PDF/A-1b 形式に準拠しているかどうかを検証すると、AEM Forms が検証エラー PDFA_CONTENT_003_DEVICE_DEPENDENT_COLOR_USED を返す。アドビのプリフライトやサードパーティソフトウェアで検証された場合、PDF ドキュメントはエラーを返さない。NPR-18011
...
* ワークフローを使用してファイルを処理するように選択すると、監視フォルダーの作成中にワークフローモデル選択ドロップダウンの表示が途切れる。NPR-17566

**HTML5 のフォーム**

* AEM Forms で、手書き署名フィールドの監査イベントがキャプチャされない。NPR-15286
...
* PDF Generator サービスを日本語 OCR と併用すると、反転した PDF が生成される。NPR-17602
...
* AEM Document Security Extension for Microsoft Office で保護されたドキュメントを Microsoft Excel や Microsoft PowerPoint で開く場合、はるかに時間がかかる。CQ-4212358
* 既存のポリシーと同じ名前の新規ポリシーが作成されると、内部サーバーエラーが発生する。NPR-18247
...
>[!NOTE]
>
>6.3.3.0 以降のリリースをインストールする場合、以下の手順を実行して JBoss アプリケーションサーバーを設定します。Oracle WebLogic または IBM WebSpehere アプリケーションサーバーで動作している AEM Forms サーバーに 6.3.3.0 をインストールする場合、追加の設定は必要ありません。詳しくは、[AEM 6.3.3.0 リリースノート](https://helpx.adobe.com/jp/experience-manager/6-3/release-notes/sp3-release-notes.html)を参照してください。
...
>[!NOTE]
>
>これらの 2 つの問題のいずれかに影響を受ける場合は、[AEM カスタマーケア](https://helpx.adobe.com/jp/marketing-cloud/contact-support.html)にお問い合わせください。
...
* adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml を開いて、次のように編集します。
