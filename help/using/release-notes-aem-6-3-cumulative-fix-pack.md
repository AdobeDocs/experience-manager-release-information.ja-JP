...
* ResourceResolver で Target コンポーネントの複数のドメインが解決されない。 NPR-28265：CQ-107847 のホットフィックス
...
* 6.2 と 6.3 の間で異なる xPath 変換が行われたことによる Query Builder の不一致NPR-23245:CQ-4240396 のホットフィックス
...
* 言語コピーの作成中、ライブコピープロパティは削除されません。 NPR-23123：CQ-4230641 のホットフィックス
...
* ContextHub が初期化中にデフォルトの永続化レイヤーを上書きするときの永続化の問題。 NPR-22479：CQ-4218399 のホットフィックス
...
* ページ上の画像コンポーネントに追加された画像を反転すると、変更が保存されず、元の画像がページに表示されます。 [https://github.com/adobe/aem-core-wcm-components/pull/141](https://github.com/adobe/aem-core-wcm-components/pull/141) を通じて画像コアコンポーネントにレンダリングサポートを追加。NPR-22801：CQ-4221539 のホットフィックス
...
* のパブリッシュインスタンスを含むページをアクティベートすると、はオレンジに変わるのではなく、緑色のステータスになります。 NPR-22927：CQ-4236310 のホットフィックス
* （スタイルシステム）ポップアップからスタイルを選択すると、スクリーンの位置がジャンプする。NPR-23183：CQ-4238867 のホットフィックス
* （公開を管理） カレンダーの翌月に移動するには、複数回のクリックが必要です。 NPR-23508：CQ-4242732 のホットフィックス

### Platform {#platform-2}

* Sling エクスポーターサーブレットで日本語の文字が正しく書き出されない。 NPR-22153：CQ-4228920 のホットフィックス
...
* Target エンジン（mbox.js、at.js）では、マッピングされた URL は使用されません。 また、では、特定のデプロイメントで失敗する可能性のあるコロンを含んだ URL を使用します。 NPR-22366：CQ-4237854 のホットフィックス
...
* ユーザーは mbox 呼び出しでテスト静的パラメーターの値を確認できません。これは、クラウド設定で AT.js をクライアントライブラリとしてテストする場合に表示されます。 NPR-21930：CQ-4234520 のホットフィックス
...
* セキュリティ向上のため、CAPTCHA コンポーネントは非推奨（廃止予定）になりました。 CAPTCHA コンポーネントを使用している場合、6.3.2.1 以降のリリースをインストールすると、「Captcha コンポーネントは非推奨のため、使用されなくなりました」というメッセージが表示されます。 セキュリティ向上のために reCAPTCHA を含めることで AEM コンポーネントをカスタマイズできます。NPR-22151：CQ-4220052 のホットフィックス

### WCM - ページエディター {#wcm-page-editor}

* 無効なセレクターを使用したときにクロスサイトスクリプティング（XSS）が反映される。 CQ-4270397 のホットフィックス
...
* PDF/A-2a またはPDF/A-3a に準拠したPDFファイルの生成がサポートされるようになりました。

### Forms アドオンパッケージ {#forms-add-on-package-9}

#### Forms - 管理 UI {#forms-admin-ui}

* ECM コネクタ設定ページでパスワードフィールドを調べると、パスワードがプレーンテキストで表示されます。 NPR-22508：CQ-4236065 のホットフィックス
...
* ルールエディターのカスタム関数で、関数呼び出しの後に関数が追加されます。 したがって、カスタム関数が true を返した場合でも、検証に失敗します。 NPR-22481：CQ-4235499 のホットフィックス
* 選択した日付パターンに関係なく、日付選択コンポーネントが、最小および最大の検証メッセージを表示する際にパターンに従わない。 NPR-22444：CQ-4236269 のホットフィックス
* 送信リクエストで送信される日付形式を、日付選択コンポーネントで指定されるパターンに合わせる必要がある。 NPR-22384
* Samsung の Android™ 6.0 デバイスでは、アダプティブフォームのテキストボックスに指定された最大文字数が無視される。NPR-22363、NPR-22364：CQ-4235205 のホットフィックス
* （Microsoft® Edge）（IE11）複数行フィールドを持つアダプティブフォームのテキストフィールドコンポーネントが、空白ではなく `Null` をデフォルト値として表示する。 NPR-22284：CQ-69107 のホットフィックス
...
* ページの公開後に変更されたFormsは、再度公開されません。 NPR-23013：CQ-4236566 のホットフィックス
...
* （HTMLWorkspace） データ準備は、ワークスペースでフォームを閉じるときにトリガーされます。 NPR-21127：CQ-4224574 のホットフィックス
* （HTML Workspace）長い説明を含むメモを必要とするプロセスを呼び出すと、メモの展開ボタンが機能しない。CQ-4241488 のホットフィックス

#### コア {#core-1}

* スケジューラの開始中、起動時にエラーが発生する。NPR-22990
* ブラウザーは、HTMLフォームに入力された資格情報を保存できません。 NPR-21762：CQ-4206543 のホットフィックス
...
* PDF/A-2a またはPDF/A-3a に準拠したPDFファイルの生成がサポートされるようになりました。 NPR-23021：CQ-4214172 のホットフィックス

#### Forms Designer {#forms-designer-1}

* DesignerからPDFとして保存すると、PDF/UA 識別子がありません。 NPR-23137
...
* Designerで定義されたインラインハイパーリンクが、DesignerからPDFとして保存されたときにタグ付けされないか、代替テキストが含まれる。 NPR-23133
...
* HSM を使用してPDFドキュメントにデジタル署名する際に例外が発生する。 NPR-21154：CQ-4226978 のホットフィックス
...
* AEM Dynamic Media Classic（旧称Scene7）で、AEM サーバーからアセットを同期できません。 この問題は、Cloud Service設定の保存先パスがルートパスのサブフォルダーを指している場合に発生します。 CQ-4228265
...
* コンテンツフラグメントに貼り付けたコンテンツの末尾に予期しないタグがあります。 NPR-20413：CQ-4224014 のホットフィックス
...
* Communities サイトを作成した後、コンテキストパスが URL の先頭に追加されないので、ユーザーはサイトマップからサイトを開くことができません。 NPR-20730
...
* Search&amp;Promoteクラウドサービスの設定では、フォーム取得方式が無視されるので、オーサーインスタンスで使用できません。 NPR-20447：CQ-4206076 のホットフィックス
* コンテンツパッケージ内のあいまいなフィルター定義が原因で、Search&amp;Promote機能がインストールされる際にパスが上書きされます。 NPR-20808

### Mobile On-Demand {#mobile-on-demand}

* TXT タイプのアセットのメタデータプロパティは、プロパティが表示されるたびに、異なるメタデータエディターに表示されます。 NPR-20239

### Platform {#platform-3}

* リソースリゾルバーが閉じられていない例外を解決。NPR-19749：Granite-19143 のホットフィックス
* オペレーションダッシュボードに、デフォルトのヘルスチェックとともにカスタムカードのリクエストを表示。NPR-20145
* Internet Explorer 11 では、ページエディターの注釈レイヤーが正しく機能しません。 NPR-20222：CQ-4222818 のホットフィックス
* レプリケーションエージェントでユーザーエージェントを管理者として設定すると、セッションリークが発生する。NPR-20578
* requestAttributes が、他の data-sly-resource ステートメントで設定解除されるようになりました。 NPR-20639：4226206 のホットフィックス
* AEM の OOTB アセットに対する curl Head リクエストの問題を修正しました。NPR-20781：CQ-4221520 のホットフィックス

### プロジェクト {#projects-1}

* プロジェクトコンソールから様々なプロジェクトにアクセスすると、読み込みに時間がかかります。 NPR-20314
...
* ダイアログボックスの属性にかかわらず、「カラー」フィールドは「常に必須」に設定されます。 NPR-19702
...
* 関連するフォームデータモデルサービスが値を返さない場合でも、ドロップダウンには最後に保存された値が表示される。 NPR-20710
...
* サブフォームなどの XDP フォームオブジェクトの名前は、アクセシビリティ設定で定義されていない場合、ツールヒントとして表示されます。 NPR-20523

### Forms JEE インストーラー {#forms-jee-installer-9}

#### プロセス管理 {#process-management-2}

* FormSetPrefillApp スタートポイントが、AEM Forms アプリケーションのフォームセットフィールドに事前入力されない。 NPR-20950
...
* Chromeを使用して、列表示でページ/アセットの完全なリストを読み込めない。 NPR-19566：CQ-4214248 のホットフィックス
...
* 設定ウィザードの「ターゲティングエンジン」フィールドが、Adobe Targetではなく ContextHub （AEM）に設定される。 NPR-19320：CQ-4218465 のホットフィックス
...
* AEM Article Console でフルサイズの画像を使用するとレンダリングが遅くなるため、問題が発生する。 NPR-19088
...
* AEM 6.3 にアップグレード後、スクリプトとして実装されたタグを参照するカスタムタグライブラリが見つからない。NPR-19087
...
* タグ管理/タグ付けコンソールからAEMからBrand Portalへのタグの公開を有効にする。 NPR-20271：CQ-4223948 のホットフィックス
...
* 「JEE に送信」ワークフローで、JEE 側からAEMに出力パラメーターが返されず、「Ignoring because the value is not primitive」エラーがスローされる。 NPR-20265
* Safari では、アダプティブフォームで PDF を添付ファイルとして使用できない。NPR-19625
* RestoreGuideState は、customcontextproperty マップを上書きします。 CQ-4222877
* Cloud Serviceを使用してGoogle `reCaptcha` を設定する場合、外部接続用に org.apache.http.proxyconfigurator の設定が必要な環境では、POSTの呼び出しがプロキシを経由しないように思われる。 NPR-20454
* ピリオド（.）以外の小数点区切り記号を持つロケールを使用したインストールで、XSD スキーマに基づくアダプティブ Formsが数値フィールドの誤った XML 値を送信し、 エラーが発生する。NPR-20444
* サードパーティサーバーに HTTP リクエストを送信する際に、「Apache HTTP Components Proxy Configuration」に設定されたプロキシ設定が順守されません。 HTTP GET または POST 呼び出しを使用した接続タイムアウトの問題。NPR-20457、NPR-20456、NPR-20455、NPR-20451
...
* フォームを最大化/最小化してドラフトとして保存または転送する場合、ワークフローの添付ファイル、「メモ」、「詳細」の各タブは、ワークスペースでは機能しません。 NPR-20243
* HTMLWorkspaceでデータを送信した後、複数行テキストフィールド（TextArea）がテキスト内の改行文字を保持しない。 NPR-20085
...
>[!NOTE]
>
>問題を修正するために、[AEM Forms リリース ](aem-forms-releases.md) の記事にリストされているLiveCycle埋め込みパッケージをインストール。
...
* アセットの必須フィールド検証が見つからないので、ドロップダウンメタデータスキーマフィールドを必須にできません。 NPR-17882：CQ-4208373 のホットフィックス
...
* プロパティページからアセットを移動すると、null ポインター例外が発生します。 CQ-4204251
* アセットが InDesign ドキュメントに複数回リンクされている場合、AEM がプロパティページでアセットへの後続の参照を追跡できない。CQ-4204186
* ハイブリッドデバイス上のChromeでの編集時にメタデータスキーマフォームに新しいタブを追加する場合の問題。 CQ-4201810
...
* アプリへの読み取り専用権限を持つユーザーが、AEM Mobile Content Management からコンテンツをプレビューできない。 NPR-17486：CQ-4209690 の CFP
* 「カタログを作成」ボタンが、カタログコンソールの列表示で機能しない。 CQ-4209952

### Sites {#sites-12}

* data-sly-resource 属性を使用した画像／ビデオコンポーネントの埋め込みに関する問題。NPR-18182：CQ-4212100 の CFP
* ライブコピーで継承を再適用した場合、変更されたローカライズされたコンポーネントが元の形式に復元されない。 NPR-18172：CQ-4211379 のホットフィックス
...
* 長いページタイトルの場合、規則に従ったページ名は生成されません。 NPR-17633：CQ-4209056 のホットフィックス
...
* Correspondence Management テキストモジュール、レタープレビュー、および Correspondence Management UI を作成するためのプログラムによる起動の修正。
* PDF Generator での PDF/A-1b 検証、大きな画像ファイルの PDF への変換、および日本語 PDF ドキュメントの修正。
* Correspondence Management、Document Security、Forms Workflowのユーザビリティが修正されました。
...
* Correspondence Management フラグメントの編集時に、テキストエディターにインライン条件と処理済みのテキストが表示されます。 CQ-4211930
...
* PDF Generatorサービスで、100 MB を超える画像をPDFドキュメントに変換できない。 CQ-4208628
* 日本語 OCR でPDF Generatorサービスを利用すると、逆PDFが発生する。 NPR-17602
...
>[!NOTE]
>
>Adobeでは、パッケージをインストールする前に、新しいインストールパッケージごとに検証を実行することをお勧めします。 事前検証は、インストール前に見つかったエラーを分析および報告し、そのようなエラーを事前に警告します。
>
>検証オプションに関するドキュメント（[https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) にアクセスできます。

* AEM 6.3.3.0 が CFP の前提条件です。AEM インストール環境をAEM 6.3 にアップグレードする手順について詳しくは、[ アップグレードドキュメント ](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) を参照してください。
...
>[!NOTE]
>
>6.3.3.0 以降のリリースをインストールする場合、次の手順を実行して JBoss® アプリケーションサーバーを設定します。 Oracle WebLogic または IBM® WebSpehere アプリケーションサーバーで動作している AEM Forms サーバーに 6.3.3.0 をインストールする場合、追加の設定は必要ありません。詳しくは、[AEM 6.3.3.0 リリースノート](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)を参照してください。
...
* AEM CFP 6.3.3.x のインストール中に次のエラーや警告が発生する場合がありますが、無視しても問題ありません。
   ...
   * org.apache.sling.engine.impl.SlingRequestProcessorImpl ServletResolver サービスが見つからず、リクエストを処理できず、ステータス 503 が送信される
...
* 6.3.1.2 以降では、スマートコレクションの「Brand PortalにPublish」ボタンが削除されました。
...
デフォルト値は 1 です。ファイル constants.js （`/libs/cq/ui/widgets/source/constants.js`）をオーバーレイして `CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL` プロパティのコメントを解除し、値をオーバーライドします。 次に、必要な値（ページプロパティのデータが格納される、ページの `/jcr:content` の下にある最大深度）を割り当てます。

**ページの `/jcr:content` ノード配下にあるノード数が 1,000 を超えるような複数のページバリアントをユーザーが作成する必要がある場合は、次の手順を使用して設定を変更します。**

* Apache Sling の JSON Max results プロパティを設定します。
* `/system/console/ configMgr` を使用したサーブレットの取得
* この値を 1000（現在のデフォルト）より大きい数値に設定し、この数が `/jcr:content` サブツリー内のノードの総数よりも大きくなるようにします。

このプロセスにより、Sling GETサーブレットは必要なすべてのノードを返すことができます。

## Uber Jar {#uber-jar}

6.3.3.8 の Uber Jar は、Adobeの Public Maven リポジトリで入手できます

Maven プロジェクトで Uber Jar を使用するには、[Uber Jar の使用方法 ](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) を参照し、プロジェクト POM に次の依存関係を含めます。
...
| 領域 | 機能 | 代替手段 | バージョン |
|----|-----|-----|-----|
| Assets と Adobe Creative Cloud の統合 | [AEM／Creative Cloud フォルダー共有](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)は、クリエイティブユーザーに AEM のアセットへのアクセスを提供する方法として、AEM 6.2 で導入されました。Creative Cloudアプリケーションでリリースされた新しい機能であるAdobeアセットリンクは、ユーザーエクスペリエンスを向上させます。 また、Photoshop、InDesign、Illustrator内からAEMのアセットに直接アクセスする、より強力な機能も提供します。</br></br> アドビでは、このフォルダー共有機能をさらに強化する予定はありません。この機能は AEM に含まれていますが、代替手段を使用することをお勧めします。 | Adobe Asset Link またはデスクトップアプリケーション。詳しくは、[AEM Creative Cloudの統合 ](https://experienceleague.adobe.com/ja/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) を参照してください。 | AEM 6.3.3.x |
...
>* [Adobe優先製品のアップデート情報に登録する ](https://www.adobe.com/subscription/priority-product-update.html)
