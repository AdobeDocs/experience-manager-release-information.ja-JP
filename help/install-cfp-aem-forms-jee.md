...
累積修正パックをAEM 6.3 にインストールするには、以下を実行します。 [!DNL Forms JEE]で、次の一連の手順を実行します。

1. AEM 6.3 を入手するには、以下を実行します。 [!DNL Forms JEE] CFP 用のインストーラー、 [Adobeサポート](https://experienceleague.adobe.com/?support-solution=General&amp;support-tab=home#support).
...
AEM [!DNL  Forms JEE] package (aemfd-jee-bundles-package-6.3CFP1;バージョン 1.0.2) では、 [!DNL Forms] AEMのユーザー [!DNL Forms JEE] AEMと同じ権限と機能 [!DNL Forms OSGi]. パッケージマネージャーでインストール済みのパッケージを確認し、パッケージがまだインストールされていない場合はインストールします。
...
累積修正パックをAEM 6.2 にインストールするには、以下を実行します。 [!DNL Forms JEE]で、次の一連の手順を実行します。

1. AEM 6.2 を入手するには、以下を実行します。 [!DNL Forms JEE] CFP 用のインストーラー、 [Adobeサポート](https://experienceleague.adobe.com/?support-solution=General&amp;support-tab=home#support).
...
AEM Forms JEE パッケージ (aemfd-jee-bundles-package-6.2CFP5;バージョン 1.0.2) では、 [!DNL Forms] AEMのユーザー [!DNL Forms JEE] AEMと同じ権限と機能 [!DNL Forms OSGi]. パッケージマネージャーでインストール済みのパッケージを確認し、パッケージがまだインストールされていない場合はインストールします。
...
DSC のデプロイメントには、失敗する可能性があるため、変数の時間がかかります。DSC 操作のタイムアウト（Install、Load、Start、Stop など）を変更するには、 `adobe.component.registry.timeout` -D オプションで JVM 引数を使用する。
...
1. 次の手順で `DSC operations`600 秒にロードしてインストールするなど、次を使用します。
...
1. インストールフォルダーを選択画面で、表示されるデフォルトの場所が既存のインストール場所であることを確認するか、または「**[!UICONTROL 参照]**[!DNL Forms]」をクリックして AEM がインストールされている別のフォルダーを選択してから、「**[!UICONTROL 次へ]**」をクリックします。
...
1. アプリケーションサーバーに応じて、以下のいずれかのドキュメントを選択し、「*AEM [!DNL Forms]* の設定とデプロイ」節の指示に従ってください。
   ...
   * AEMのインストールとデプロイ [!DNL Forms] (WebSphere®の場合 )
